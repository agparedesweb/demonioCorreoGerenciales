...
Namespace Comunes.Comun
    ...
    Public Class ClsTools
        ...
        Private Shared archivoIni As IniFileController.IniFileController
...
#End Region
        ...
        Public Shared Function fgObtenParametroEMB(ByRef prmClavePar As String, ByVal prmSucursal As String) As Object
        ...
        Public Shared Function fgObtenCommoditieName(ByVal prmDescripProduct As String) As String

            Dim DT As New DataTable
            Dim vcSQL As String

            fgObtenCommoditieName = ""

            vcSQL = "SELECT cCommodityName FROM EYE_COMMODITIESMARENGO WHERE CPRODUCTDESCRIP = '" & prmDescripProduct & "'"
            ...
            If Not DT Is Nothing Then
                ...
                If DT.Rows.Count > 0 Then
                    fgObtenCommoditieName = DT.Rows(0)("cCommodityName")
                End If
            End If
...
#End Region
        ...
        Public Shared Sub CreaConfiguracionDeMaquina()
            ...
            archivoIni = New IniFileController.IniFileController
        ...
        Private Shared Sub CargaParametrosADDA()
            Dim DAO As DataAccessCls = DataAccessCls.DevuelveInstancia
            Dim vIFCarchivoIni As New IniFileController.IniFileController
...
#End Region
        ...
        Public Shared Function fgObtenParametroGeneral(ByRef prmTabla As String, ByVal prmCampo As String) As Object
            ...
            Try
                vcSQL = "SELECT " & prmCampo & " AS RESULTADO FROM " & prmTabla
                ...
                If Not DT Is Nothing Then
                    ...
                    If DT.Rows.Count > 0 Then
                        fgObtenParametroGeneral = DT.Rows(0)("RESULTADO")
                    End If
                End If
        ...
        Public Shared Function fgObtenParametroMail(ByRef prmClavePar As String) As Object
        ...
        Public Shared Function fgGetParametro(ByRef prmClavePar As String) As Object
            ...
            vcSQL = "SELECT CVALORPAR FROM EYE_PARAMETROS WHERE CCLAVEPAR = '" & prmClavePar & "'"
            ...
            If Not DT Is Nothing Then
                ...
                If DT.Rows.Count > 0 Then
                    fgGetParametro = DT.Rows(0)("CVALORPAR")
                End If
            End If
        ...
        Public Shared Function fgFechaMediana(ByVal Fecha As Date, Optional ByVal vlAnoCompleto As Boolean = True) As String
            'Sistema    : Rutinas Generales de Fechas
            'Módulo     : Parametrización
            'Programa   : fgFechaMediana
            'Desarrollo : Roque Montoya
            'Fecha      : 18 Junio del 2001
            'Que Hace?  : Regresa una cadena tipo string en el cual lleva la descripción de una
            '             fecha. Ejemplo:
            '                        -El usuario manda la fecha '2001-12-25'
            '                        -La funcion regresaria : "25/Dic/2001"
            'Parámetros : Fecha .- Variable de tipo Fecha en el cual el usuario mandara como
            '             parametro para saber su descripción con palabras.
            Dim vlCMes As String
            ...
            fgFechaMediana = Trim(Str(Fecha.Day)) + "/" + Trim(vlCMes) + "/" + Right(Trim(Str(Year(Fecha))), IIf(vlAnoCompleto, 4, 2))

        End Function


        Public Shared Function fgYaSeEnvioMailCliente(ByRef prmCliente As String, ByVal prmHorario As String, ByVal prmFecha As Date) As Boolean

            Dim DT As New DataTable
            Dim vcSQL As String

            fgYaSeEnvioMailCliente = False

            vcSQL = "SELECT * FROM EYE_FECHASCORREOS(NOLOCK)"
            vcSQL = vcSQL & vbCrLf & "WHERE CCVE_CLIENTE = '" & prmCliente & "' AND DFECHA = '" & Format(prmFecha, "yyyyMMdd") & "'"
            vcSQL = vcSQL & vbCrLf & "AND CHORARIO = '" & prmHorario & "'"
        ...
        Public Shared Sub pgObtenMailClientes(ByRef prmCorreoHM As String, ByRef prmCorreoMarengo As String, ByVal prmClienteHM As String, ByVal prmClienteMarengo As String)

            Dim DT As New DataTable
            Dim vcSQL As String

            vcSQL = "SELECT * FROM CTL_CLIENTES(NOLOCK)"
            vcSQL = vcSQL & vbCrLf & "WHERE CCVE_CLIENTE IN ('" & prmClienteHM & "')"
            ...
            If Not DT Is Nothing Then
                ...
                If DT.Rows.Count > 0 Then
                    prmCorreoHM = DT.Rows(0)("CEMAIL")
                End If
            End If

            vcSQL = "SELECT * FROM CTL_CLIENTES(NOLOCK)"
            vcSQL = vcSQL & vbCrLf & "WHERE CCVE_CLIENTE IN ('" & prmClienteMarengo & "')"
            ...
            If Not DT Is Nothing Then
                ...
                If DT.Rows.Count > 0 Then
                    prmCorreoMarengo = DT.Rows(0)("CEMAIL")
                End If
            End If
        ...
        Public Shared Function fgCreaManifiestoHM(ByVal txtcEMBARQUE As String, ByVal prmTemporada As String, ByVal prmEmpaque As String, ByVal vldirec As String, ByVal prmRutaXLS As String) As String
            ...
            Try
                ...
                txtcEMBARQUE = fgPonceros(txtcEMBARQUE, 8)

                vcSQL.AppendLine("SELECT CFOLIO_REMIS,C_SCAC,CONVERT(CHAR(10),EE.DFECHA_TRABAJO,111)+' '+ CONVERT(CHAR(5),EE.DFECHA_TRABAJO,114) AS FECHA,EE.NCAJA,EE.CPLACASCAJA,CE.CNOMBRE,EE.CNOM_CHOFER AS CHOFER,EE.NCAMION,EE.CPLACASCAMION,CLICENCIA,CSELLOORIGEN+CASE WHEN LTRIM(RTRIM(CSELLOREPOSICION))='' THEN '' ELSE '-'+LTRIM(RTRIM(CSELLOREPOSICION)) END+CASE WHEN LTRIM(RTRIM(CSELLOLATERAL))='' THEN '' ELSE '-'+LTRIM(RTRIM(CSELLOLATERAL)) END AS SELLOS")
                vcSQL.AppendLine("FROM EYE_ENCEMBARQUES EE(NOLOCK)")
                vcSQL.AppendLine("LEFT JOIN CTL_LINEASDETRANSPORTE CL(NOLOCK) ON CL.CCVE_LINEA=EE.CCVE_LINEA")
                vcSQL.AppendLine("LEFT JOIN CTL_CHOFERES CC(NOLOCK) ON CC.CCVE_CHOFER=EE.CCVE_CHOFER")
                vcSQL.AppendLine("LEFT JOIN CTL_EMBARCADORES CE(NOLOCK) ON CE.CCVE_EMBARCADOR=EE.CCVE_EMBARCADOR")
                vcSQL.AppendLine("WHERE EE.CFOLIO_REMIS='" & fgPonceros(txtcEMBARQUE, 8) & "' AND EE.CSTATUS='A' AND EE.CCVE_TEMPORADA='" & prmTemporada & "' and EE.CCVE_EMPAQUE='" & prmEmpaque & "'")

                DAO.RegresaConsultaSQL(vcSQL.ToString, DTDetalle)

                If DTDetalle Is Nothing Then
                    fgCreaManifiestoHM = "No Existe el Manifiesto [" & txtcEMBARQUE & "] ó a sido Cancelado"
                    Exit Function
                End If

                'xlLibro = xlApp.Workbooks.Open("\\192.168.2.21\crop\EXES" & "\MANIFIESTOS HM.XLS")
                xlLibro = xlApp.Workbooks.Open(prmRutaXLS)

                xlHoja = xlLibro.Worksheets.Application.Sheets("SPV")
                ...
                With xlHoja
                    ...
                    If Not DTCabecero Is Nothing AndAlso DTDetalle.Rows.Count > 0 Then
                        .Cells(4, "F").Value = Val(DTCabecero.Rows(0)("CFOLIO_REMIS"))
                        .Cells(4, "Q").Value = DTCabecero.Rows(0)("C_SCAC")
                        .Cells(5, "F").Value = DTCabecero.Rows(0)("Fecha")
                        .Cells(5, "Q").Value = DTCabecero.Rows(0)("NCAJA")
                        .Cells(5, "U").Value = DTCabecero.Rows(0)("CPLACASCAJA")
                        .Cells(6, "F").Value = DTCabecero.Rows(0)("cnombre")
                        .Cells(6, "Q").Value = DTCabecero.Rows(0)("CHOFER")
                        .Cells(7, "F").Value = DTCabecero.Rows(0)("NCAMION")
                        .Cells(7, "N").Value = DTCabecero.Rows(0)("CPLACASCAMION")
                        .Cells(7, "R").Value = DTCabecero.Rows(0)("CLICENCIA")
                        .Cells(8, "R").Value = DTCabecero.Rows(0)("SELLOS")
                    End If
                    ...
                    vcSQL.AppendLine("SELECT")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '006' THEN DE.NBULTOS ELSE 0 END),0) AS SPV2TAN3X4,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '007' THEN DE.NBULTOS ELSE 0 END),0) AS SPV2TAN4X4,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '008' THEN DE.NBULTOS ELSE 0 END),0) AS SPV2TAN4X5,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '009' THEN DE.NBULTOS ELSE 0 END),0) AS SPV2TAN5X5,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '010' THEN DE.NBULTOS ELSE 0 END),0) AS SPV2TAN5X6,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '011' THEN DE.NBULTOS ELSE 0 END),0) AS SPV2TAN6X6,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '012' THEN DE.NBULTOS ELSE 0 END),0) AS SPV2TAN6X7,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '006' THEN DE.NBULTOS ELSE 0 END),0) AS PAR2TAN3X4,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '007' THEN DE.NBULTOS ELSE 0 END),0) AS PAR2TAN4X4,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '008' THEN DE.NBULTOS ELSE 0 END),0) AS PAR2TAN4X5,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '009' THEN DE.NBULTOS ELSE 0 END),0) AS PAR2TAN5X5,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '010' THEN DE.NBULTOS ELSE 0 END),0) AS PAR2TAN5X6,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '011' THEN DE.NBULTOS ELSE 0 END),0) AS PAR2TAN6X6,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '011' AND DE.CCVE_TAMAÑO = '012' THEN DE.NBULTOS ELSE 0 END),0) AS PAR2TAN6X7,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '006' THEN DE.NBULTOS ELSE 0 END),0) AS SPV25LBS3X4,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '007' THEN DE.NBULTOS ELSE 0 END),0) AS SPV25LBS4X4,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '008' THEN DE.NBULTOS ELSE 0 END),0) AS SPV25LBS4X5,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '009' THEN DE.NBULTOS ELSE 0 END),0) AS SPV25LBS5X5,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '010' THEN DE.NBULTOS ELSE 0 END),0) AS SPV25LBS5X6,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '011' THEN DE.NBULTOS ELSE 0 END),0) AS SPV25LBS6X6,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '012' THEN DE.NBULTOS ELSE 0 END),0) AS SPV25LBS6X7,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '006' THEN DE.NBULTOS ELSE 0 END),0) AS PAR25LBS3X4,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '007' THEN DE.NBULTOS ELSE 0 END),0) AS PAR25LBS4X4,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '008' THEN DE.NBULTOS ELSE 0 END),0) AS PAR25LBS4X5,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '009' THEN DE.NBULTOS ELSE 0 END),0) AS PAR25LBS5X5,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '010' THEN DE.NBULTOS ELSE 0 END),0) AS PAR25LBS5X6,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '011' THEN DE.NBULTOS ELSE 0 END),0) AS PAR25LBS6X6,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_ENVASE = '001' AND DE.CCVE_TAMAÑO = '012' THEN DE.NBULTOS ELSE 0 END),0) AS PAR25LBS6X7")
                    vcSQL.AppendLine(" ")
                    vcSQL.AppendLine("FROM EYE_ENCEMBARQUES EE(NOLOCK)")
                    vcSQL.AppendLine(" LEFT JOIN EYE_DETEMBARQUES DE(NOLOCK)")
                    vcSQL.AppendLine(" ON EE.CCVE_TEMPORADA=DE.CCVE_TEMPORADA")
                    vcSQL.AppendLine(" AND EE.CCVE_EMPAQUE=DE.CCVE_EMPAQUE")
                    vcSQL.AppendLine(" AND EE.CFOLIO_MANIF=DE.CFOLIO_MANIF")
                    vcSQL.AppendLine(" LEFT JOIN CTL_CULTIVOS CC(NOLOCK)")
                    vcSQL.AppendLine(" ON  DE.CCVE_CULTIVO=CC.CCVE_CULTIVO")
                    vcSQL.AppendLine(" LEFT JOIN CTL_TAMAÑOS CT(NOLOCK)")
                    vcSQL.AppendLine(" ON CT.CCVE_CULTIVO=DE.CCVE_CULTIVO AND CT.CCVE_TAMAÑO=DE.CCVE_TAMAÑO")
                    vcSQL.AppendLine(" WHERE EE.CFOLIO_REMIS='" & fgPonceros(txtcEMBARQUE, 8) & "' AND EE.CSTATUS='A' AND EE.CCVE_TEMPORADA='" & prmTemporada & "' AND DE.CCVE_CULTIVO IN ('002') and EE.CCVE_EMPAQUE='" & prmEmpaque & "'  AND CSOBREPESO='N'")
                    ...
                    If Not DTCabecero Is Nothing AndAlso DTCabecero.Rows.Count > 0 Then
                        ...
                        ''' 2 TANDAS SPV
                        .Cells(12, "C").Value = IIf(DTCabecero.Rows(0)("SPV2TAN3X4") <> 0, DTCabecero.Rows(0)("SPV2TAN3X4"), "")
                        .Cells(13, "C").Value = IIf(DTCabecero.Rows(0)("SPV2TAN4X4") <> 0, DTCabecero.Rows(0)("SPV2TAN4X4"), "")
                        .Cells(14, "C").Value = IIf(DTCabecero.Rows(0)("SPV2TAN4X5") <> 0, DTCabecero.Rows(0)("SPV2TAN4X5"), "")
                        .Cells(15, "C").Value = IIf(DTCabecero.Rows(0)("SPV2TAN5X5") <> 0, DTCabecero.Rows(0)("SPV2TAN5X5"), "")
                        .Cells(16, "C").Value = IIf(DTCabecero.Rows(0)("SPV2TAN5X6") <> 0, DTCabecero.Rows(0)("SPV2TAN5X6"), "")
                        .Cells(17, "C").Value = IIf(DTCabecero.Rows(0)("SPV2TAN6X6") <> 0, DTCabecero.Rows(0)("SPV2TAN6X6"), "")
                        .Cells(18, "C").Value = IIf(DTCabecero.Rows(0)("SPV2TAN6X7") <> 0, DTCabecero.Rows(0)("SPV2TAN6X7"), "")

                        '' 2 TANDAS PARIS
                        .Cells(12, "E").Value = IIf(DTCabecero.Rows(0)("PAR2TAN3X4") <> 0, DTCabecero.Rows(0)("PAR2TAN3X4"), "")
                        .Cells(13, "E").Value = IIf(DTCabecero.Rows(0)("PAR2TAN4X4") <> 0, DTCabecero.Rows(0)("PAR2TAN4X4"), "")
                        .Cells(14, "E").Value = IIf(DTCabecero.Rows(0)("PAR2TAN4X5") <> 0, DTCabecero.Rows(0)("PAR2TAN4X5"), "")
                        .Cells(15, "E").Value = IIf(DTCabecero.Rows(0)("PAR2TAN5X5") <> 0, DTCabecero.Rows(0)("PAR2TAN5X5"), "")
                        .Cells(16, "E").Value = IIf(DTCabecero.Rows(0)("PAR2TAN5X6") <> 0, DTCabecero.Rows(0)("PAR2TAN5X6"), "")
                        .Cells(17, "E").Value = IIf(DTCabecero.Rows(0)("PAR2TAN6X6") <> 0, DTCabecero.Rows(0)("PAR2TAN6X6"), "")
                        .Cells(18, "E").Value = IIf(DTCabecero.Rows(0)("PAR2TAN6X7") <> 0, DTCabecero.Rows(0)("PAR2TAN6X7"), "")


                        ''' 25 LIBRAS SPV
                        .Cells(12, "G").Value = IIf(DTCabecero.Rows(0)("SPV25LBS3X4") <> 0, DTCabecero.Rows(0)("SPV25LBS3X4"), "")
                        .Cells(13, "G").Value = IIf(DTCabecero.Rows(0)("SPV25LBS4X4") <> 0, DTCabecero.Rows(0)("SPV25LBS4X4"), "")
                        .Cells(14, "G").Value = IIf(DTCabecero.Rows(0)("SPV25LBS4X5") <> 0, DTCabecero.Rows(0)("SPV25LBS4X5"), "")
                        .Cells(15, "G").Value = IIf(DTCabecero.Rows(0)("SPV25LBS5X5") <> 0, DTCabecero.Rows(0)("SPV25LBS5X5"), "")
                        .Cells(16, "G").Value = IIf(DTCabecero.Rows(0)("SPV25LBS5X6") <> 0, DTCabecero.Rows(0)("SPV25LBS5X6"), "")
                        .Cells(17, "G").Value = IIf(DTCabecero.Rows(0)("SPV25LBS6X6") <> 0, DTCabecero.Rows(0)("SPV25LBS6X6"), "")
                        .Cells(18, "G").Value = IIf(DTCabecero.Rows(0)("SPV25LBS6X7") <> 0, DTCabecero.Rows(0)("SPV25LBS6X7"), "")


                        ''' 25 LIBRAS PAR
                        .Cells(12, "I").Value = IIf(DTCabecero.Rows(0)("PAR25LBS3X4") <> 0, DTCabecero.Rows(0)("PAR25LBS3X4"), "")
                        .Cells(13, "I").Value = IIf(DTCabecero.Rows(0)("PAR25LBS4X4") <> 0, DTCabecero.Rows(0)("PAR25LBS4X4"), "")
                        .Cells(14, "I").Value = IIf(DTCabecero.Rows(0)("PAR25LBS4X5") <> 0, DTCabecero.Rows(0)("PAR25LBS4X5"), "")
                        .Cells(15, "I").Value = IIf(DTCabecero.Rows(0)("PAR25LBS5X5") <> 0, DTCabecero.Rows(0)("PAR25LBS5X5"), "")
                        .Cells(16, "I").Value = IIf(DTCabecero.Rows(0)("PAR25LBS5X6") <> 0, DTCabecero.Rows(0)("PAR25LBS5X6"), "")
                        .Cells(17, "I").Value = IIf(DTCabecero.Rows(0)("PAR25LBS6X6") <> 0, DTCabecero.Rows(0)("PAR25LBS6X6"), "")
                        .Cells(18, "I").Value = IIf(DTCabecero.Rows(0)("PAR25LBS6X7") <> 0, DTCabecero.Rows(0)("PAR25LBS6X7"), "")
                    ...
                    'ROMAS
                    vcSQL.AppendLine("SELECT")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '032' THEN DE.NBULTOS ELSE 0 END),0) AS SPVJMB,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '002' THEN DE.NBULTOS ELSE 0 END),0) AS SPVXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '003' THEN DE.NBULTOS ELSE 0 END),0) AS SPVLAR,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '004' THEN DE.NBULTOS ELSE 0 END),0) AS SPVMED,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '005' THEN DE.NBULTOS ELSE 0 END),0) AS SPVSMALL,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '032' THEN DE.NBULTOS ELSE 0 END),0) AS PARJMB,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '002' THEN DE.NBULTOS ELSE 0 END),0) AS PARXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '003' THEN DE.NBULTOS ELSE 0 END),0) AS PARLAR,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '004' THEN DE.NBULTOS ELSE 0 END),0) AS PARMED,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '005' THEN DE.NBULTOS ELSE 0 END),0) AS PARSMALL")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("FROM EYE_ENCEMBARQUES EE(NOLOCK)")
                    vcSQL.AppendLine("LEFT JOIN EYE_DETEMBARQUES DE(NOLOCK)")
                    vcSQL.AppendLine("ON EE.CCVE_TEMPORADA=DE.CCVE_TEMPORADA")
                    ...
                    vcSQL.AppendLine("LEFT JOIN CTL_TAMAÑOS CT(NOLOCK)")
                    vcSQL.AppendLine("ON CT.CCVE_CULTIVO=DE.CCVE_CULTIVO AND CT.CCVE_TAMAÑO=DE.CCVE_TAMAÑO")
                    vcSQL.AppendLine(" WHERE EE.CFOLIO_REMIS='" & fgPonceros(txtcEMBARQUE, 8) & "' AND EE.CSTATUS='A' AND EE.CCVE_TEMPORADA='" & prmTemporada & "' AND DE.CCVE_CULTIVO IN ('004') and EE.CCVE_EMPAQUE='" & prmEmpaque & "'  AND CSOBREPESO='N'")
                    ...
                    If Not DTCabecero Is Nothing AndAlso DTCabecero.Rows.Count > 0 Then
                        ...
                        ''' SPV
                        .Cells(31, "F").Value = IIf(DTCabecero.Rows(0)("SPVJMB") <> 0, DTCabecero.Rows(0)("SPVJMB"), "")
                        .Cells(32, "F").Value = IIf(DTCabecero.Rows(0)("SPVXL") <> 0, DTCabecero.Rows(0)("SPVXL"), "")
                        .Cells(33, "F").Value = IIf(DTCabecero.Rows(0)("SPVLAR") <> 0, DTCabecero.Rows(0)("SPVLAR"), "")
                        .Cells(34, "F").Value = IIf(DTCabecero.Rows(0)("SPVMED") <> 0, DTCabecero.Rows(0)("SPVMED"), "")
                        .Cells(35, "F").Value = IIf(DTCabecero.Rows(0)("SPVSMALL") <> 0, DTCabecero.Rows(0)("SPVSMALL"), "")

                        '' PARIS
                        .Cells(31, "K").Value = IIf(DTCabecero.Rows(0)("PARJMB") <> 0, DTCabecero.Rows(0)("PARJMB"), "")
                        .Cells(32, "K").Value = IIf(DTCabecero.Rows(0)("PARXL") <> 0, DTCabecero.Rows(0)("PARXL"), "")
                        .Cells(33, "K").Value = IIf(DTCabecero.Rows(0)("PARLAR") <> 0, DTCabecero.Rows(0)("PARLAR"), "")
                        .Cells(34, "K").Value = IIf(DTCabecero.Rows(0)("PARMED") <> 0, DTCabecero.Rows(0)("PARMED"), "")
                        .Cells(35, "K").Value = IIf(DTCabecero.Rows(0)("PARSMALL") <> 0, DTCabecero.Rows(0)("PARSMALL"), "")
                    ...
                    'BERENJENA
                    vcSQL.AppendLine("SELECT")
                    vcSQL.AppendLine("-- SPV NO RPC")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '024' AND DE.CCVE_ENVASE NOT IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPV16,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '025' AND DE.CCVE_ENVASE NOT IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPV18,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '026' AND DE.CCVE_ENVASE NOT IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPV24,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '027' AND DE.CCVE_ENVASE NOT IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPV32,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '029' AND DE.CCVE_ENVASE NOT IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPV40,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("-- PARIS NO RPC")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '024' AND DE.CCVE_ENVASE NOT IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS PAR16,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '025' AND DE.CCVE_ENVASE NOT IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS PAR18,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '026' AND DE.CCVE_ENVASE NOT IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS PAR24,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '027' AND DE.CCVE_ENVASE NOT IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS PAR32,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '029' AND DE.CCVE_ENVASE NOT IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS PAR40,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("-- RPC")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_TAMAÑO = '024' AND DE.CCVE_ENVASE IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS RPC16,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_TAMAÑO = '025' AND DE.CCVE_ENVASE IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS RPC18,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_TAMAÑO = '026' AND DE.CCVE_ENVASE IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS RPC24,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_TAMAÑO = '027' AND DE.CCVE_ENVASE IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS RPC32,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_TAMAÑO = '029' AND DE.CCVE_ENVASE IN ('024','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS RPC40")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("FROM EYE_ENCEMBARQUES EE(NOLOCK)")
                    vcSQL.AppendLine("LEFT JOIN EYE_DETEMBARQUES DE(NOLOCK)")
                    vcSQL.AppendLine("ON EE.CCVE_TEMPORADA=DE.CCVE_TEMPORADA")
                    ...
                    vcSQL.AppendLine("LEFT JOIN CTL_TAMAÑOS CT(NOLOCK)")
                    vcSQL.AppendLine("ON CT.CCVE_CULTIVO=DE.CCVE_CULTIVO AND CT.CCVE_TAMAÑO=DE.CCVE_TAMAÑO")
                    vcSQL.AppendLine(" WHERE EE.CFOLIO_REMIS='" & fgPonceros(txtcEMBARQUE, 8) & "' AND EE.CSTATUS='A' AND EE.CCVE_TEMPORADA='" & prmTemporada & "' AND DE.CCVE_CULTIVO IN ('005') and EE.CCVE_EMPAQUE='" & prmEmpaque & "'  AND CSOBREPESO='N'")
                    ...
                    If Not DTCabecero Is Nothing AndAlso DTCabecero.Rows.Count > 0 Then
                        ...
                        ''' SPV
                        .Cells(41, "G").Value = IIf(DTCabecero.Rows(0)("SPV16") <> 0, DTCabecero.Rows(0)("SPV16"), "")
                        .Cells(42, "G").Value = IIf(DTCabecero.Rows(0)("SPV18") <> 0, DTCabecero.Rows(0)("SPV18"), "")
                        .Cells(43, "G").Value = IIf(DTCabecero.Rows(0)("SPV24") <> 0, DTCabecero.Rows(0)("SPV24"), "")
                        .Cells(44, "G").Value = IIf(DTCabecero.Rows(0)("SPV32") <> 0, DTCabecero.Rows(0)("SPV32"), "")
                        .Cells(45, "G").Value = IIf(DTCabecero.Rows(0)("SPV40") <> 0, DTCabecero.Rows(0)("SPV40"), "")

                        '' PARIS
                        .Cells(41, "K").Value = IIf(DTCabecero.Rows(0)("PAR16") <> 0, DTCabecero.Rows(0)("PAR16"), "")
                        .Cells(42, "K").Value = IIf(DTCabecero.Rows(0)("PAR18") <> 0, DTCabecero.Rows(0)("PAR18"), "")
                        .Cells(43, "K").Value = IIf(DTCabecero.Rows(0)("PAR24") <> 0, DTCabecero.Rows(0)("PAR24"), "")
                        .Cells(44, "K").Value = IIf(DTCabecero.Rows(0)("PAR32") <> 0, DTCabecero.Rows(0)("PAR32"), "")
                        .Cells(45, "K").Value = IIf(DTCabecero.Rows(0)("PAR40") <> 0, DTCabecero.Rows(0)("PAR40"), "")

                        '' RPC
                        .Cells(41, "N").Value = IIf(DTCabecero.Rows(0)("RPC16") <> 0, DTCabecero.Rows(0)("RPC16"), "")
                        .Cells(42, "N").Value = IIf(DTCabecero.Rows(0)("RPC18") <> 0, DTCabecero.Rows(0)("RPC18"), "")
                        .Cells(43, "N").Value = IIf(DTCabecero.Rows(0)("RPC24") <> 0, DTCabecero.Rows(0)("RPC24"), "")
                        .Cells(44, "N").Value = IIf(DTCabecero.Rows(0)("RPC32") <> 0, DTCabecero.Rows(0)("RPC32"), "")
                        .Cells(45, "N").Value = IIf(DTCabecero.Rows(0)("RPC40") <> 0, DTCabecero.Rows(0)("RPC40"), "")
                    ...
                    ' CHILES VERDES
                    vcSQL.AppendLine("SELECT")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '036' AND DE.CCVE_ENVASE NOT IN ('024','026','027','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVJMB,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '002' AND DE.CCVE_ENVASE NOT IN ('024','026','027','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '003' AND DE.CCVE_ENVASE NOT IN ('024','026','027','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVLAR,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '004' AND DE.CCVE_ENVASE NOT IN ('024','026','027','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVMED,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '005' AND DE.CCVE_ENVASE NOT IN ('024','026','027','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVSMALL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '043' AND DE.CCVE_ENVASE NOT IN ('024','026','027','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVXXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '042' AND DE.CCVE_ENVASE NOT IN ('024','026','027','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVXSMALL,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '036' THEN DE.NBULTOS ELSE 0 END),0) AS PARJMB,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '002' THEN DE.NBULTOS ELSE 0 END),0) AS PARXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '003' THEN DE.NBULTOS ELSE 0 END),0) AS PARLAR,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '004' THEN DE.NBULTOS ELSE 0 END),0) AS PARMED,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '005' THEN DE.NBULTOS ELSE 0 END),0) AS PARSMALL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '043' THEN DE.NBULTOS ELSE 0 END),0) AS PARXXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='003' AND DE.CCVE_TAMAÑO = '042' THEN DE.NBULTOS ELSE 0 END),0) AS PARXSMALL,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '036' AND DE.CCVE_ENVASE IN ('024','026','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVRPCJMB,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '002' AND DE.CCVE_ENVASE IN ('024','026','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVRPCXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '003' AND DE.CCVE_ENVASE IN ('024','026','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVRPCLAR,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '004' AND DE.CCVE_ENVASE IN ('024','026','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVRPCMED,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '005' AND DE.CCVE_ENVASE IN ('024','026','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVRPCSMALL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '043' AND DE.CCVE_ENVASE IN ('024','026','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVRPCXXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '042' AND DE.CCVE_ENVASE IN ('024','026','034','035') THEN DE.NBULTOS ELSE 0 END),0) AS SPVRPCXSMALL,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '036' AND DE.CCVE_ENVASE IN ('027') THEN DE.NBULTOS ELSE 0 END),0) AS SPVEUROJMB,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '002' AND DE.CCVE_ENVASE IN ('027') THEN DE.NBULTOS ELSE 0 END),0) AS SPVEUROXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '003' AND DE.CCVE_ENVASE IN ('027') THEN DE.NBULTOS ELSE 0 END),0) AS SPVEUROLAR,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '004' AND DE.CCVE_ENVASE IN ('027') THEN DE.NBULTOS ELSE 0 END),0) AS SPVEUROMED,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '005' AND DE.CCVE_ENVASE IN ('027') THEN DE.NBULTOS ELSE 0 END),0) AS SPVEUROSMALL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '043' AND DE.CCVE_ENVASE IN ('027') THEN DE.NBULTOS ELSE 0 END),0) AS SPVEUROXXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_ETIQUETA='001' AND DE.CCVE_TAMAÑO = '042' AND DE.CCVE_ENVASE IN ('027') THEN DE.NBULTOS ELSE 0 END),0) AS SPVEUROXSMALL")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("FROM EYE_ENCEMBARQUES EE(NOLOCK)")
                    vcSQL.AppendLine("LEFT JOIN EYE_DETEMBARQUES DE(NOLOCK)")
                    vcSQL.AppendLine("ON EE.CCVE_TEMPORADA=DE.CCVE_TEMPORADA")
                    ...
                    vcSQL.AppendLine("LEFT JOIN CTL_TAMAÑOS CT(NOLOCK)")
                    vcSQL.AppendLine("ON CT.CCVE_CULTIVO=DE.CCVE_CULTIVO AND CT.CCVE_TAMAÑO=DE.CCVE_TAMAÑO")
                    vcSQL.AppendLine(" WHERE EE.CFOLIO_REMIS='" & fgPonceros(txtcEMBARQUE, 8) & "' AND EE.CSTATUS='A' AND EE.CCVE_TEMPORADA='" & prmTemporada & "' AND DE.CCVE_CULTIVO IN ('003') and EE.CCVE_EMPAQUE='" & prmEmpaque & "'  AND CSOBREPESO='N'")
                    ...
                    If Not DTCabecero Is Nothing AndAlso DTCabecero.Rows.Count > 0 Then
                        ...
                        ''' SPV
                        .Cells(12, "P").Value = IIf(DTCabecero.Rows(0)("SPVXXL") <> 0, DTCabecero.Rows(0)("SPVXXL"), "")
                        .Cells(13, "P").Value = IIf(DTCabecero.Rows(0)("SPVXL") <> 0, DTCabecero.Rows(0)("SPVXL"), "")
                        .Cells(14, "P").Value = IIf(DTCabecero.Rows(0)("SPVLAR") <> 0, DTCabecero.Rows(0)("SPVLAR"), "")
                        .Cells(15, "P").Value = IIf(DTCabecero.Rows(0)("SPVMED") <> 0, DTCabecero.Rows(0)("SPVMED"), "")
                        .Cells(16, "P").Value = IIf(DTCabecero.Rows(0)("SPVSMALL") <> 0, DTCabecero.Rows(0)("SPVSMALL"), "")
                        .Cells(17, "P").Value = IIf(DTCabecero.Rows(0)("SPVXSMALL") <> 0, DTCabecero.Rows(0)("SPVXSMALL"), "")

                        '' PARIS
                        .Cells(12, "Q").Value = IIf(DTCabecero.Rows(0)("PARJMB") <> 0, DTCabecero.Rows(0)("PARXXL"), "")
                        .Cells(13, "Q").Value = IIf(DTCabecero.Rows(0)("PARXL") <> 0, DTCabecero.Rows(0)("PARXL"), "")
                        .Cells(14, "Q").Value = IIf(DTCabecero.Rows(0)("PARLAR") <> 0, DTCabecero.Rows(0)("PARLAR"), "")
                        .Cells(15, "Q").Value = IIf(DTCabecero.Rows(0)("PARMED") <> 0, DTCabecero.Rows(0)("PARMED"), "")
                        .Cells(16, "Q").Value = IIf(DTCabecero.Rows(0)("PARSMALL") <> 0, DTCabecero.Rows(0)("PARSMALL"), "")
                        .Cells(17, "Q").Value = IIf(DTCabecero.Rows(0)("PARXSMALL") <> 0, DTCabecero.Rows(0)("PARXSMALL"), "")

                        ''' SPV RPC
                        .Cells(12, "T").Value = IIf(DTCabecero.Rows(0)("SPVRPCXXL") <> 0, DTCabecero.Rows(0)("SPVRPCXXL"), "")
                        .Cells(13, "T").Value = IIf(DTCabecero.Rows(0)("SPVRPCXL") <> 0, DTCabecero.Rows(0)("SPVRPCXL"), "")
                        .Cells(14, "T").Value = IIf(DTCabecero.Rows(0)("SPVRPCLAR") <> 0, DTCabecero.Rows(0)("SPVRPCLAR"), "")
                        .Cells(15, "T").Value = IIf(DTCabecero.Rows(0)("SPVRPCMED") <> 0, DTCabecero.Rows(0)("SPVRPCMED"), "")
                        .Cells(16, "T").Value = IIf(DTCabecero.Rows(0)("SPVRPCSMALL") <> 0, DTCabecero.Rows(0)("SPVRPCSMALL"), "")
                        .Cells(17, "T").Value = IIf(DTCabecero.Rows(0)("SPVRPCXSMALL") <> 0, DTCabecero.Rows(0)("SPVRPCXSMALL"), "")

                        ''' EURO SPV
                        .Cells(12, "U").Value = IIf(DTCabecero.Rows(0)("SPVEUROXXL") <> 0, DTCabecero.Rows(0)("SPVEUROXXL"), "")
                        .Cells(13, "U").Value = IIf(DTCabecero.Rows(0)("SPVEUROXL") <> 0, DTCabecero.Rows(0)("SPVEUROXL"), "")
                        .Cells(14, "U").Value = IIf(DTCabecero.Rows(0)("SPVEUROLAR") <> 0, DTCabecero.Rows(0)("SPVEUROLAR"), "")
                        .Cells(15, "U").Value = IIf(DTCabecero.Rows(0)("SPVEUROMED") <> 0, DTCabecero.Rows(0)("SPVEUROMED"), "")
                        .Cells(16, "U").Value = IIf(DTCabecero.Rows(0)("SPVEUROSMALL") <> 0, DTCabecero.Rows(0)("SPVEUROSMALL"), "")
                        .Cells(17, "U").Value = IIf(DTCabecero.Rows(0)("SPVEUROXSMALL") <> 0, DTCabecero.Rows(0)("SPVEUROXSMALL"), "")
                    ...
                    ' CHILES DE COLORES
                    vcSQL.AppendLine("SELECT")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='008' AND DE.CCVE_TAMAÑO = '011' THEN DE.NBULTOS ELSE 0 END),0) AS REDJMB,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='008' AND DE.CCVE_TAMAÑO = '002' THEN DE.NBULTOS ELSE 0 END),0) AS REDXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='008' AND DE.CCVE_TAMAÑO = '003' THEN DE.NBULTOS ELSE 0 END),0) AS REDLAR,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='008' AND DE.CCVE_TAMAÑO = '004' THEN DE.NBULTOS ELSE 0 END),0) AS REDMED,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='008' AND DE.CCVE_TAMAÑO = '005' THEN DE.NBULTOS ELSE 0 END),0) AS REDSMALL,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='061' AND DE.CCVE_TAMAÑO = '008' THEN DE.NBULTOS ELSE 0 END),0) AS YELJMB,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='061' AND DE.CCVE_TAMAÑO = '002' THEN DE.NBULTOS ELSE 0 END),0) AS YELXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='061' AND DE.CCVE_TAMAÑO = '003' THEN DE.NBULTOS ELSE 0 END),0) AS YELLAR,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='061' AND DE.CCVE_TAMAÑO = '004' THEN DE.NBULTOS ELSE 0 END),0) AS YELMED,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='061' AND DE.CCVE_TAMAÑO = '005' THEN DE.NBULTOS ELSE 0 END),0) AS YELSMALL,")
                    vcSQL.AppendLine("")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='010' AND DE.CCVE_TAMAÑO = '008' THEN DE.NBULTOS ELSE 0 END),0) AS NARJMB,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='010' AND DE.CCVE_TAMAÑO = '002' THEN DE.NBULTOS ELSE 0 END),0) AS NARXL,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='010' AND DE.CCVE_TAMAÑO = '003' THEN DE.NBULTOS ELSE 0 END),0) AS NARLAR,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='010' AND DE.CCVE_TAMAÑO = '004' THEN DE.NBULTOS ELSE 0 END),0) AS NARMED,")
                    vcSQL.AppendLine("COALESCE(SUM(CASE WHEN DE.CCVE_CULTIVO='010' AND DE.CCVE_TAMAÑO = '005' THEN DE.NBULTOS ELSE 0 END),0) AS NARSMALL")
                    vcSQL.AppendLine("FROM EYE_ENCEMBARQUES EE(NOLOCK)")
                    vcSQL.AppendLine("LEFT JOIN EYE_DETEMBARQUES DE(NOLOCK)")
                    vcSQL.AppendLine("ON EE.CCVE_TEMPORADA=DE.CCVE_TEMPORADA")
                    vcSQL.AppendLine("AND EE.CCVE_EMPAQUE=DE.CCVE_EMPAQUE")
                    ...
                    vcSQL.AppendLine("LEFT JOIN CTL_TAMAÑOS CT(NOLOCK)")
                    vcSQL.AppendLine("ON CT.CCVE_CULTIVO=DE.CCVE_CULTIVO AND CT.CCVE_TAMAÑO=DE.CCVE_TAMAÑO")
                    vcSQL.AppendLine(" WHERE EE.CFOLIO_REMIS='" & fgPonceros(txtcEMBARQUE, 8) & "' AND EE.CSTATUS='A' AND EE.CCVE_TEMPORADA='" & prmTemporada & "' AND DE.CCVE_CULTIVO IN ('008','010','061') and EE.CCVE_EMPAQUE='" & prmEmpaque & "'  AND CSOBREPESO='N'")
                    ...
                    If Not DTCabecero Is Nothing AndAlso DTCabecero.Rows.Count > 0 Then
                        ...
                        ''' RED
                        .Cells(25, "P").Value = IIf(DTCabecero.Rows(0)("REDJMB") <> 0, DTCabecero.Rows(0)("REDJMB"), "")
                        .Cells(26, "P").Value = IIf(DTCabecero.Rows(0)("REDXL") <> 0, DTCabecero.Rows(0)("REDXL"), "")
                        .Cells(27, "P").Value = IIf(DTCabecero.Rows(0)("REDLAR") <> 0, DTCabecero.Rows(0)("REDLAR"), "")
                        .Cells(28, "P").Value = IIf(DTCabecero.Rows(0)("REDMED") <> 0, DTCabecero.Rows(0)("REDMED"), "")
                        .Cells(29, "P").Value = IIf(DTCabecero.Rows(0)("REDSMALL") <> 0, DTCabecero.Rows(0)("REDSMALL"), "")

                        '' YELLOW
                        .Cells(25, "Q").Value = IIf(DTCabecero.Rows(0)("YELJMB") <> 0, DTCabecero.Rows(0)("YELJMB"), "")
                        .Cells(26, "Q").Value = IIf(DTCabecero.Rows(0)("YELXL") <> 0, DTCabecero.Rows(0)("YELXL"), "")
                        .Cells(27, "Q").Value = IIf(DTCabecero.Rows(0)("YELLAR") <> 0, DTCabecero.Rows(0)("YELLAR"), "")
                        .Cells(28, "Q").Value = IIf(DTCabecero.Rows(0)("YELMED") <> 0, DTCabecero.Rows(0)("YELMED"), "")
                        .Cells(29, "Q").Value = IIf(DTCabecero.Rows(0)("YELSMALL") <> 0, DTCabecero.Rows(0)("YELSMALL"), "")

                        '' ORANGE
                        .Cells(25, "T").Value = IIf(DTCabecero.Rows(0)("NARJMB") <> 0, DTCabecero.Rows(0)("NARJMB"), "")
                        .Cells(26, "T").Value = IIf(DTCabecero.Rows(0)("NARXL") <> 0, DTCabecero.Rows(0)("NARXL"), "")
                        .Cells(27, "T").Value = IIf(DTCabecero.Rows(0)("NARLAR") <> 0, DTCabecero.Rows(0)("NARLAR"), "")
                        .Cells(28, "T").Value = IIf(DTCabecero.Rows(0)("NARMED") <> 0, DTCabecero.Rows(0)("NARMED"), "")
                        .Cells(29, "T").Value = IIf(DTCabecero.Rows(0)("NARSMALL") <> 0, DTCabecero.Rows(0)("NARSMALL"), "")
                    ...
                    'TOTAL PALLETS
                    vcSQL.AppendLine("SELECT COUNT(*) AS TOTAL")
                    vcSQL.AppendLine("FROM EYE_ENCEMBARQUES EE(NOLOCK)")
                    vcSQL.AppendLine("LEFT JOIN EYE_DETEMBARQUES DE(NOLOCK)")
                    vcSQL.AppendLine("ON EE.CCVE_TEMPORADA=DE.CCVE_TEMPORADA AND EE.CCVE_EMPAQUE=DE.CCVE_EMPAQUE AND EE.CFOLIO_MANIF=DE.CFOLIO_MANIF")
                    vcSQL.AppendLine("WHERE EE.CFOLIO_REMIS='" & fgPonceros(txtcEMBARQUE, 8) & "' AND EE.CSTATUS='A' AND EE.CCVE_TEMPORADA='" & prmTemporada & "' AND EE.CCVE_EMPAQUE='" & prmEmpaque & "'  AND CSOBREPESO='N'")
                    vcSQL.AppendLine("GROUP BY EE.CFOLIO_REMIS")
                    vcSQL.AppendLine("ORDER BY EE.CFOLIO_REMIS")

                    DAO.RegresaConsultaSQL(vcSQL.ToString, DTCabecero)

                    If Not DTCabecero Is Nothing AndAlso DTCabecero.Rows.Count > 0 Then
                        .Cells(51, "G").Value = DTCabecero.Rows(0)("total")
                    End If
                    DTCabecero = Nothing
        ...
        Public Shared Function fgGeneraFactExp(ByVal prmRuta As String, ByVal prmFolioFac As String, ByVal prmRutaXLS As String) As String
            '***********************************************************
            ' ROSALES ROLDAN JOSE ALFREDO
            ' 09 / MAR / 2007
            ' LLENAR MANIFIESTO H.M.
            ...
            Try
                ...
                Dim CULTIVO As String
                Dim I As Integer
                Dim vldirec As String
                Dim VLCFOLIO_MANIF As String
                Dim vnTotalKilos As Decimal
                VLCFOLIO_MANIF = ""
                vlsql = "select CFOLIO_MANIF from vista_fact_enc where cfolio_fac=" & Trim(prmFolioFac) & " and left(clave,8)='" & fgGetParametro("TEMPORADA") & fgGetParametro("EMPAQUE") & "' AND CMERCADO = 'E'"

                DAO.RegresaConsultaSQL(vlsql, DT)

                If Not DT Is Nothing AndAlso DT.Rows.Count > 0 Then
                    VLCFOLIO_MANIF = DT.Rows(0)("CFOLIO_MANIF")
                Else
                    Exit Function
                End If
                DT = Nothing
                ...
                'xlLibro = xlApp.Workbooks.Open("\\Serverparedes\CROP\EXES" & "\FACTURA.XLS")
                xlLibro = xlApp.Workbooks.Open(prmRutaXLS)

                xlHoja = xlLibro.Worksheets.Application.Sheets("FACTURA")

                With xlHoja
                    'ENCABEZADOS
                    vlsql = "select * from VISTA_FACTURA_EXCEL where CLAVE='" & fgGetParametro("TEMPORADA") & fgGetParametro("EMPAQUE") & VLCFOLIO_MANIF & "E' AND cfolio_fac=" & Trim(Trim(prmFolioFac))

                    DAO.RegresaConsultaSQL(vlsql, DTDetalle)

                    If Not DTDetalle Is Nothing AndAlso DTDetalle.Rows.Count > 0 Then
                        .Cells(4, "F").Value = Val(DTDetalle.Rows(0)("cfolio_fac"))
                        .Cells(7, "F").Value = Format(DTDetalle.Rows(0)("DFECHA_TRABAJO"), "dd/MM/yyyy")
                        .Cells(8, "C").Value = Trim(DTDetalle.Rows(0)("CLIENTEF"))
                        .Cells(9, "C").Value = Trim(DTDetalle.Rows(0)("COLONIA"))
                        .Cells(10, "C").Value = Trim(DTDetalle.Rows(0)("DOMICILIO"))

                        .Cells(11, "C").Value = "CULIACAN, SIN." & ", MEXICO"
                        If Trim(DTDetalle.Rows(0)("CMERCADO")) = "N" Then
                            .Cells(32, "C").Value = fgNumLetraNormal(Val(DTDetalle.Rows(0)("Subtotal")))
                        Else
                            .Cells(32, "C").Value = fgNumLetraUS(Val(DTDetalle.Rows(0)("Subtotal")))
                        End If
                        .Cells(36, "D").Value = Val(DTDetalle.Rows(0)("CFOLIO_REMIS"))
                        .Cells(37, "D").Value = Trim(DTDetalle.Rows(0)("NCAJA"))
                        .Cells(35, "G").Value = DTDetalle.Rows(0)("Subtotal")
                        .Cells(38, "B").Value = DTDetalle.Rows(0)("NTAMANOCAJA")
                        .Cells(39, "B").Value = DTDetalle.Rows(0)("CCAAT")

                    End If
                    DTDetalle = New DataTable

                    vlsql = "SELECT V.*"
                    vlsql = vlsql + vbCrLf + " FROM EYE_ENCEMBARQUES EE(NOLOCK)"
                    vlsql = vlsql + vbCrLf + " JOIN vista_fact_DET V ON EE.CCVE_TEMPORADA+EE.CCVE_EMPAQUE+EE.CFOLIO_MANIF+EE.CMERCADO=CLAVE"
                    vlsql = vlsql + vbCrLf + " where V.CLAVE='" & fgGetParametro("TEMPORADA") & fgGetParametro("EMPAQUE") & VLCFOLIO_MANIF & "E' AND EE.cfolio_fac=" & Trim(prmFolioFac)

                    DAO.RegresaConsultaSQL(vlsql, DTDetalle)

                    I = 15
                    Dim vnTotalCajas As Long
                    Dim TOTAL_BULTOS As Long
                    Dim vnTotalRPC As Long

                    vnTotalCajas = 0
                    vnTotalRPC = 0

                    If Not DTDetalle Is Nothing AndAlso DTDetalle.Rows.Count > 0 Then
                        For Each vRow As DataRow In DTDetalle.Rows

                            .Cells(I, "A").Value = Val(vRow("NBULTOS"))
                            .Cells(I, "B").Value = Trim(vRow("CONTENIDO"))
                            .Cells(I, "E").Value = Trim(vRow("NPRECIO"))
                            .Cells(I, "G").Value = Trim(vRow("nimporte"))
                            TOTAL_BULTOS = TOTAL_BULTOS + Val(vRow("NBULTOS"))

                            vnTotalKilos = vnTotalKilos + Val(vRow("NKGS"))

                            If Trim(vRow("CESRPC")) = "N" Then
                                vnTotalCajas += Val(vRow("NBULTOS"))
                            Else
                                vnTotalRPC += Val(vRow("NBULTOS"))
                            End If
                    ...
                    .Cells(I, "A").Value = "_____________"
                    I = I + 1
                    .Cells(I, "A").Value = TOTAL_BULTOS

                    If vnTotalCajas > 0 Then
                        I = I + 1
                        .Cells(I, "A").Value = vnTotalCajas
                        .Cells(I, "B").Value = "CAJAS QUE SE RETORNAN DE ACUERDO AL PROGRAMA IMMEX"
                    End If

                End With

                xlHoja.Cells(40, "B").Value = Format(vnTotalKilos, "###,###")
                vldirec = prmRuta
                xlHoja.SaveAs(vldirec)
                xlLibro.Close()
                xlApp.Quit()
            ...
            Catch ex As Exception
                fgGeneraFactExp = ex.Message
            End Try

        End Function


        Public Shared Function fgCreaVistasGuia(ByVal prmFolioRemis As String, ByVal prmManifiesto As String) As String
            ...
            Try
                ...
                vcSQL.AppendLine("SELECT ccve_temporada + ccve_empaque + cfolio_manif + eee.cmercado                                                             AS clave, ")
                vcSQL.AppendLine("       CASE  ")
                vcSQL.AppendLine("         WHEN ccve_moneda = '02' THEN eee.cimporteus ")
                vcSQL.AppendLine("         ELSE '' ")
                vcSQL.AppendLine("       END                                                    AS cimporteus, ")
                vcSQL.AppendLine("       ch.cnombre                                                                                                              AS chofer, ")
                vcSQL.AppendLine("       ch.clicencia                                                                                                            AS licencia, ")
                vcSQL.AppendLine("       eee.cfolio_manif                                                                                                        AS cfolio_manif, ")
                vcSQL.AppendLine("       eee.cfolio_fac                                                                                                          AS cfolio_fac, ")
                vcSQL.AppendLine("       eee.cfolio_remis                                                                                                        AS cfolio_remis, ")
                vcSQL.AppendLine("       eee.dfecha_trabajo, ")
                vcSQL.AppendLine("       Cast(Datename(MONTH,eee.dfecha_trabajo) AS CHAR(3))                                                                     AS mes, ")
                vcSQL.AppendLine("       Cast(Year(eee.dfecha_trabajo) AS CHAR(4))                                                                               AS anio, ")
                vcSQL.AppendLine("       Cast(Day(eee.dfecha_trabajo) AS CHAR(2))                                                                                AS dia, ")
                vcSQL.AppendLine("       eee.cplacascaja, ")
                vcSQL.AppendLine("       lt.cnombre                                                                                                              AS lineatransp, ")
                vcSQL.AppendLine("       tr.cnombre                                                                                                              AS transporte, ")
                vcSQL.AppendLine("       eee.ncamion, ")
                vcSQL.AppendLine("       eee.cplacascamion, ")
                vcSQL.AppendLine("       eee.cmarcatransporte, ")
                vcSQL.AppendLine("       ce.cnombrecomer                                                                                                         AS embarcador, ")
                vcSQL.AppendLine("       ce.crfc                                                                                                                 AS rfc_embarcador, ")
                vcSQL.AppendLine("       Convert(VARCHAR(8),eee.dfecha_trabajo,108)                                                                              AS horasalida, ")
                vcSQL.AppendLine("       Isnull(ce.cdireccion,'') + '   ' + Isnull(ce.ccolonia,'')                                                               AS dir_embarcador, ")
                vcSQL.AppendLine("       Isnull(ce.ccolonia,'')                                                                                                  AS colonia_emb, ")
                vcSQL.AppendLine("       ce.cciudad, ")
                vcSQL.AppendLine("       ce.ccodigopostal, ")
                vcSQL.AppendLine("       sc.crfc                                                                                                                 AS rfc_cliente, ")
                vcSQL.AppendLine("       sc.cnombre                                                                                                              AS cliente, ")
                vcSQL.AppendLine("       sc.cnombre, ")
                vcSQL.AppendLine("       sc.domicilio, ")
                vcSQL.AppendLine("       sc.domicilio                                                                                                            AS domicilio_cte, ")
                vcSQL.AppendLine("       sc.domicilio                                                                                                            AS domicilio_cte2, ")
                vcSQL.AppendLine("       sc.colonia                                                                                                              AS colonia, ")
                vcSQL.AppendLine("       Cast(Isnull(ce.cdireccion,'') AS CHAR(60))                                                                              AS domicilio_guia, ")
                vcSQL.AppendLine("       Substring(ce.ccolonia,2 + Patindex('%,%',Isnull(ce.ccolonia,'')), ")
                vcSQL.AppendLine("                 Len(Isnull(ce.ccolonia,'')))               AS municipio_prod, ")
                vcSQL.AppendLine("       sc.ciudad, ")
                vcSQL.AppendLine("       sc.ctelefono, ")
                vcSQL.AppendLine("       eee.nbultos_total, ")
                vcSQL.AppendLine("       c.cnombrecomer                                                                                                          AS clientecomerf, ")
                vcSQL.AppendLine("       c.cnombre                                                                                                               AS clientef, ")
                vcSQL.AppendLine("       c.crfc                                                                                                                  AS frfc, ")
                vcSQL.AppendLine("       c.ciudad                                                                                                                AS ciudadf, ")
                vcSQL.AppendLine("       c.ccodigopostal                                                                                                         AS codigopostalf, ")
                vcSQL.AppendLine("       c.domicilio                                                                                                             AS domiciliof, ")
                vcSQL.AppendLine("       eee.nimporte                                                                                                            AS subtotal, ")
                vcSQL.AppendLine("       eee.nimporte_seg                                                                                                        AS seguro, ")
                vcSQL.AppendLine("       (eee.nimporte_seg * 0.15)                                                                                               AS iva_seguro, ")
                vcSQL.AppendLine("       Isnull(eee.nimporte,0) + Isnull(eee.nimporte_seg,0) + (Isnull(eee.nimporte_seg,0) * 0.15) + Isnull(npreciotermografo,0) AS total, ")
                vcSQL.AppendLine("       ce.ctelefono                                                                                                            AS tel_emb, ")
                vcSQL.AppendLine("       Isnull(npreciotermografo,0) + (SELECT Sum(nimporte) ")
                vcSQL.AppendLine("                                      FROM   eye_costosembarques AS ece(nolock) ")
                vcSQL.AppendLine("                                      WHERE  ece.ccve_temporada = eee.ccve_temporada ")
                vcSQL.AppendLine("                                             AND ece.ccve_empaque = eee.ccve_empaque ")
                vcSQL.AppendLine("                                             AND ece.cfolio_manif = eee.cfolio_manif ")
                vcSQL.AppendLine("                                             AND ece.cmercado = eee.cmercado) AS otros, ")
                vcSQL.AppendLine("       eee.ntemptermografo, ")
                vcSQL.AppendLine("       eee.cnumtermografo, ")
                vcSQL.AppendLine("       eee.nimporteflete, ")
                vcSQL.AppendLine("       eee.nanticipoflete, ")
                vcSQL.AppendLine("       npreciotermografo, ")
                vcSQL.AppendLine("       eee.cjefe_empaques, ")
                vcSQL.AppendLine("       ad.cnombre                                                                                                              AS aduana, ")
                vcSQL.AppendLine("       CASE  ")
                vcSQL.AppendLine("         WHEN civa = '1' THEN '+ I.V.A.' ")
                vcSQL.AppendLine("         ELSE '' ")
                vcSQL.AppendLine("       END                                                                AS iva, ")
                vcSQL.AppendLine("       (SELECT cvalorpar ")
                vcSQL.AppendLine("        FROM   eye_parametros (NOLOCK) ")
                vcSQL.AppendLine("        WHERE  cclavepar = 'LEYENDA_EMB') AS leyenda, ")
                vcSQL.AppendLine("       cobservaciones, ")
                vcSQL.AppendLine("       'LOTE-PAB-' + Right(eee.cfolio_remis,4)                                                                                 AS lotepab, ")
                vcSQL.AppendLine("       (CASE  ")
                vcSQL.AppendLine("          WHEN eee.cmercado = 'N' THEN 'LOTE-PAB-No. REM-' + Right(eee.cfolio_remis,4) ")
                vcSQL.AppendLine("          ELSE 'LOT-PAB-' + Right(eee.cfolio_remis,4) ")
                vcSQL.AppendLine("        END) AS lotetipo, ")
                vcSQL.AppendLine("       c_scac, ")
                vcSQL.AppendLine("       csusp_agreement, ")
                vcSQL.AppendLine("       cnum_economia, ")
                vcSQL.AppendLine("       cfacility_rnumber, ")
                vcSQL.AppendLine("       eee.cnom_chofer                                                                                                         AS cnom_chofer, ")
                vcSQL.AppendLine("       eee.cnom_chofer2                                                                                                        AS cnom_chofer2, ")
                vcSQL.AppendLine("       cselloorigen, ")
                vcSQL.AppendLine("       cselloreposicion, ")
                vcSQL.AppendLine("       csellolateral, ")
                vcSQL.AppendLine("       caades, ")
                vcSQL.AppendLine("       (cnom_ciudad + ', ' + cestado)                                                                                          AS destination, ")
                vcSQL.AppendLine("       eee.cmodelotransporte, ")
                vcSQL.AppendLine("       Dateadd(MONTH,1,eee.dfecha_trabajo)                                                                                     AS liquidacion, ")
                vcSQL.AppendLine("       eee.ncaja, ")
                vcSQL.AppendLine("       sc.domicilio                                                                                                            AS domicilio_ex, ")
                vcSQL.AppendLine("       sc.ciudad_fac, ")
                vcSQL.AppendLine("       CASE  ")
                vcSQL.AppendLine("         WHEN c.csupermercado = 'S' THEN c.domicilio_nota ")
                vcSQL.AppendLine("         ELSE sc.domicilio ")
                vcSQL.AppendLine("       END                                     AS domicilio_nota, ")
                vcSQL.AppendLine("       CASE  ")
                vcSQL.AppendLine("         WHEN c.csupermercado = 'S' THEN c.ccolonia ")
                vcSQL.AppendLine("         ELSE sc.colonia ")
                vcSQL.AppendLine("       END                                             AS colonia_nota, ")
                vcSQL.AppendLine("       CASE  ")
                vcSQL.AppendLine("         WHEN c.csupermercado = 'S' THEN c.delegacion ")
                vcSQL.AppendLine("         ELSE '' ")
                vcSQL.AppendLine("       END                                                   AS delegacion, ")
                vcSQL.AppendLine("       CASE  ")
                vcSQL.AppendLine("         WHEN c.csupermercado = 'S' THEN eee.nkgs_total ")
                vcSQL.AppendLine("         ELSE 0 ")
                vcSQL.AppendLine("       END                                                  AS nkgs_total, ")
                vcSQL.AppendLine("       CASE  ")
                vcSQL.AppendLine("         WHEN Ltrim(Rtrim(cordencompra)) <> '' THEN 'ORDEN DE COMPRA: ' + cordencompra ")
                vcSQL.AppendLine("         ELSE '' ")
                vcSQL.AppendLine("       END                  AS ordencompra, ")
                vcSQL.AppendLine("       CASE  ")
                vcSQL.AppendLine("         WHEN Ltrim(Rtrim(cfolionota)) <> '' THEN 'FOLIO DE CITA: ' + cfolionota ")
                vcSQL.AppendLine("         ELSE '' ")
                vcSQL.AppendLine("       END                        AS foliocita, ")
                vcSQL.AppendLine("       eee.nimporte AS importe_nac,")
                vcSQL.AppendLine("       eee.nimporte AS importe_nac_2,")
                vcSQL.AppendLine("       ch.ctelefono                                                                                                            AS ctelchofer, ")
                vcSQL.AppendLine("       Coalesce(eee.ntamanocaja,0)                                                                                             AS ntamanocaja, ")
                vcSQL.AppendLine("       Coalesce(eee.ccaat,'')                                                                                                  AS ccaat, ")
                vcSQL.AppendLine("       Coalesce(eee.ntemperatura,0)                                                                                            AS ntemperatura, ")
                vcSQL.AppendLine("       Coalesce(eee.cmercado,'')                                                                                            AS cmercado ")
                vcSQL.AppendLine("FROM   eye_encembarques AS eee(readpast) ")
                vcSQL.AppendLine("       LEFT JOIN vista_embarcadores_ciudades AS ce(nolock) ")
                vcSQL.AppendLine("         ON ce.ccve_embarcador = eee.ccve_embarcador ")
                vcSQL.AppendLine("       LEFT JOIN vista_clientes_sucursales AS sc(nolock) ")
                vcSQL.AppendLine("         ON (eee.ccve_cliente = sc.ccve_cliente ")
                vcSQL.AppendLine("             AND eee.ccve_sucursal = sc.ccve_sucursal) ")
                vcSQL.AppendLine("       LEFT JOIN vista_clientes AS c(nolock) ")
                vcSQL.AppendLine("         ON (eee.ccve_cliente = c.ccve_cliente) ")
                vcSQL.AppendLine("       LEFT JOIN ctl_choferes AS ch(nolock) ")
                vcSQL.AppendLine("         ON eee.ccve_chofer = ch.ccve_chofer ")
                vcSQL.AppendLine("       LEFT JOIN ctl_lineasdetransporte AS lt(nolock) ")
                vcSQL.AppendLine("         ON eee.ccve_linea = lt.ccve_linea ")
                vcSQL.AppendLine("       LEFT JOIN ctl_transportes AS tr(nolock) ")
                vcSQL.AppendLine("         ON eee.ccve_transporte = tr.ccve_transporte ")
                vcSQL.AppendLine("       LEFT JOIN ctl_aduanas AS ad(nolock) ")
                vcSQL.AppendLine("         ON eee.ccve_aduana = ad.ccve_aduana")
                vcSQL.AppendLine("WHERE eee.CFOLIO_REMIS = '" & prmFolioRemis & "'")
                vcSQL.AppendLine("AND eee.CCVE_TEMPORADA = '" & fgGetParametro("TEMPORADA") & "'")
                vcSQL.AppendLine("AND eee.CMERCADO = 'E'")


                If Not fgCreaVista(vcSQL.ToString, "VWENCGUIA") Then
                    fgCreaVistasGuia = "Ocurrio un error"
                    Exit Function
                End If
                ...
                vcSQL.AppendLine("SELECT   e.ccve_temporada + e.ccve_empaque + e.cfolio_manif + e.cmercado AS clave, ")
                vcSQL.AppendLine("         ceb.caades                                                      AS caarc, ")
                vcSQL.AppendLine("         c.cnombre + ' ' + ce.cnombre                                    AS cultivo, ")
                vcSQL.AppendLine("         Sum(e.nbultos)                                                  AS nbultos, ")

                'vcSQL.AppendLine("         Sum(e.nkgs)                                                     AS toneladas ")
                vcSQL.AppendLine("         Sum(e.NBULTOS * ceb.KGSXBULTO)                                  As toneladas ")

                vcSQL.AppendLine("FROM     eye_detembarques AS e ")
                vcSQL.AppendLine("         --       LEFT JOIN CTL_TAMAÑOS AS T(NOLOCK) ON E.CCVE_TAMA?O=T.CCVE_TAMAÑO ")
                vcSQL.AppendLine("         INNER JOIN eye_encembarques enc(nolock)")
                vcSQL.AppendLine("         ON ENC.CCVE_TEMPORADA=E.CCVE_TEMPORADA AND ENC.CCVE_EMPAQUE=E.CCVE_EMPAQUE AND ENC.CFOLIO_MANIF = E.CFOLIO_MANIF")
                vcSQL.AppendLine("         INNER JOIN ctl_cultivos AS c(nolock) ")
                vcSQL.AppendLine("           ON c.ccve_cultivo = e.ccve_cultivo ")
                vcSQL.AppendLine("         INNER JOIN ctl_envases ce(nolock) ")
                vcSQL.AppendLine("           ON ce.ccve_envase = e.ccve_envase ")
                vcSQL.AppendLine("         INNER JOIN ctl_envase_bultos ceb(nolock) ")
                vcSQL.AppendLine("           ON ceb.ccve_envase = e.ccve_envase ")
                vcSQL.AppendLine("              AND ceb.ccve_cultivo = e.ccve_cultivo ")
                vcSQL.AppendLine("WHERE    e.csobrepeso = 'N' ")
                vcSQL.AppendLine("AND ENC.CFOLIO_REMIS = '" & prmFolioRemis & "'")
                vcSQL.AppendLine("AND ENC.CCVE_TEMPORADA = '" & fgGetParametro("TEMPORADA") & "'")
                vcSQL.AppendLine("AND ENC.CMERCADO = 'E'")
                vcSQL.AppendLine("GROUP BY e.ccve_temporada, ")
                vcSQL.AppendLine("         e.ccve_empaque, ")
                vcSQL.AppendLine("         e.cfolio_manif, ")
                vcSQL.AppendLine("         c.cnombre, ")
                vcSQL.AppendLine("         ce.cnombre, ")
                vcSQL.AppendLine("         ceb.caades, ")
                vcSQL.AppendLine("         e.cmercado")


                If Not fgCreaVista(vcSQL.ToString, "VWDETGUIA") Then
                    fgCreaVistasGuia = "Ocurrio un error"
                    Exit Function
                End If
                ...
                vcSQL.AppendLine("SELECT   clave, ")
                vcSQL.AppendLine("         Count(* )                      AS npalet, ")
                vcSQL.AppendLine("         (Sum(nimporte) / Sum(nbultos)) AS npromedio, ")
                vcSQL.AppendLine("         Sum(nbultos)                   AS totalcant, ")
                vcSQL.AppendLine("         Sum(nimporte)                  AS totalimporte, ")
                vcSQL.AppendLine("         Sum(nkgs)                      AS total_toneladas ")
                vcSQL.AppendLine("FROM     vista_manif_det ")
                vcSQL.AppendLine("WHERE CLAVE = '" & fgGetParametro("TEMPORADA") & fgGetParametro("EMPAQUE") & prmManifiesto & "E" & "'")
                vcSQL.AppendLine("GROUP BY clave")


                If Not fgCreaVista(vcSQL.ToString, "VWTOTALGUIA") Then
                    fgCreaVistasGuia = "Ocurrio un error"
                    Exit Function
                End If
                ...
                vcSQL.AppendLine("SELECT EDE.CCVE_TEMPORADA+EDE.CCVE_EMPAQUE+EDE.CFOLIO_MANIF+EDE.CMERCADO AS CLAVE,")
                vcSQL.AppendLine("SUM( EDE.NBULTOS) AS NBULTOS, ET.CNOMBRE AS ETIQUETA,C.CNOMBRE AS CULTIVO,EN.CABREVIATURA AS ENVASES,")
                vcSQL.AppendLine("TA.CNOMBRE AS TAMAÑO")
                vcSQL.AppendLine("FROM EYE_DETEMBARQUES AS EDE(READPAST)")
                vcSQL.AppendLine("INNER JOIN eye_encembarques enc(nolock)")
                vcSQL.AppendLine("ON ENC.CCVE_TEMPORADA=EDE.CCVE_TEMPORADA AND ENC.CCVE_EMPAQUE=EDE.CCVE_EMPAQUE AND ENC.CFOLIO_MANIF = EDE.CFOLIO_MANIF")
                vcSQL.AppendLine("LEFT JOIN CTL_TAMAÑOS AS TA(NOLOCK) ON EDE.CCVE_TAMAÑO=TA.CCVE_TAMAÑO and TA.ccve_cultivo=ede.ccve_cultivo")
                vcSQL.AppendLine("LEFT JOIN CTL_ETIQUETAS AS ET(NOLOCK) ON ET.CCVE_ETIQUETA=EDE.CCVE_ETIQUETA")
                vcSQL.AppendLine("LEFT JOIN CTL_CULTIVOS AS C(NOLOCK) ON C.CCVE_CULTIVO=EDE.CCVE_CULTIVO AND C.CCVE_CULTIVO=TA.CCVE_CULTIVO")
                vcSQL.AppendLine("LEFT JOIN CTL_ENVASES AS EN(NOLOCK) ON EN.CCVE_ENVASE=EDE.CCVE_ENVASE")
                vcSQL.AppendLine("WHERE EDE.CSOBREPESO='N'")
                vcSQL.AppendLine("AND ENC.CFOLIO_REMIS = '" & prmFolioRemis & "'")
                vcSQL.AppendLine("AND ENC.CCVE_TEMPORADA = '" & fgGetParametro("TEMPORADA") & "'")
                vcSQL.AppendLine("AND ENC.CMERCADO = 'E'")
                vcSQL.AppendLine("GROUP BY EDE.CCVE_TEMPORADA,EDE.CCVE_EMPAQUE,EDE.CFOLIO_MANIF,EDE.CMERCADO,TA.CNOMBRE, ")
                vcSQL.AppendLine("ET.CNOMBRE,C.CNOMBRE,EN.CABREVIATURA ")

                If Not fgCreaVista(vcSQL.ToString, "VWDETALLEMANEXP") Then
                    fgCreaVistasGuia = "Ocurrio un error"
                    Exit Function
                End If
                ...
                vcSQL.AppendLine("  SELECT   clave, ")
                vcSQL.AppendLine("           Sum(npallets)                  AS npalet, ")
                vcSQL.AppendLine("           (Sum(nimporte) / Sum(nbultos)) AS npromedio, ")
                vcSQL.AppendLine("           Sum(nbultos)                   AS totalcant, ")
                vcSQL.AppendLine("           Sum(nimporte)                  AS totalimporte, ")
                vcSQL.AppendLine("           Sum(nkgs)                      AS total_toneladas ")
                ...
                If Not fgCreaVista(vcSQL.ToString, "VWTOTALESMANEXP") Then
                    fgCreaVistasGuia = "Ocurrio un error"
                    Exit Function
                End If
                ...
                vcSQL.AppendLine("SELECT EDE.CCVE_TEMPORADA+EDE.CCVE_EMPAQUE+EDE.CFOLIO_MANIF+EDE.CMERCADO AS CLAVE ,     ")
                vcSQL.AppendLine("    SUM(EDE.NBULTOS) AS NBULTOS,CASE WHEN CC.CSUPERMERCADO='S' THEN SUM(EDE.NKGS) ELSE 0 END AS NKGS,")
                vcSQL.AppendLine("    SUM(EDE.NIMPORTE) AS NIMPORTE,")
                vcSQL.AppendLine("    U.CDESCRIP AS CUNIDAD, U.CABREVIAT AS CUNIDADAB,")
                vcSQL.AppendLine("    NCANTIDAD=(CASE WHEN U.CDESCRIP='KILOGRAMOS' THEN SUM(EDE.NKGS) ELSE SUM(EDE.NBULTOS) END),")
                vcSQL.AppendLine("    NPRECIO=CONVERT(NUMERIC(10,2),(CASE WHEN U.CDESCRIP='KILOGRAMOS' THEN (SUM(EDE.NIMPORTE)/SUM(EDE.NKGS)) ELSE (SUM(EDE.NIMPORTE)/SUM(EDE.NBULTOS)) END))     ")
                vcSQL.AppendLine("    ,C.CNOMBRE+' '+CT.CNOMBRE+' '+CE.CNOMBRE +' '+ETI.CNOMBRE AS CONTENIDO,CASE WHEN CC.CSUPERMERCADO='S' THEN SUM(EDE.NIMPORTE)/SUM(EDE.NKGS) ELSE 0 END AS PRECIO_NKGS")
                vcSQL.AppendLine("FROM EYE_DETEMBARQUES AS EDE(READPAST)")
                vcSQL.AppendLine("left join eye_encembarques as ee(NOLOCK) ON EDE.CCVE_TEMPORADA=EE.CCVe_TEMPORADA AND EDE.CCVE_EMPAQUE=EE.CCVE_EMPAQUE AND EDE.CFOLIO_MANIF=EE.CFOLIO_MANIF AND EDE.CMERCADO=EE.CMERCADO")
                vcSQL.AppendLine("LEFT JOIN CTL_CLIENTES CC(NOLOCK) ON CC.CCVE_CLIENTE=EE.CCVE_CLIENTE")
                vcSQL.AppendLine("LEFT JOIN UNIDADES AS U(NOLOCK) ON U.CUNIDAD=EDE.CCVE_UNIDAD_PCIO")
                vcSQL.AppendLine("LEFT JOIN CTL_CULTIVOS AS C(NOLOCK) ON C.CCVE_CULTIVO=EDE.CCVE_CULTIVO")
                vcSQL.AppendLine("LEFT join CTL_TAMAÑOS ct(NOLOCK) ON ct.ccve_tamaño=ede.ccve_tamaño and ct.ccve_cultivo=ede.ccve_cultivo")
                vcSQL.AppendLine("LEFT JOIN CTL_ETIQUETAS ETI(NOLOCK) ON ETI.CCVE_ETIQUETA=EDE.CCVE_ETIQUETA")
                vcSQL.AppendLine("LEFT JOIN CTL_ENVASES CE(NOLOCK) ON CE.CCVE_ENVASE=EDE.CCVE_ENVASE")
                vcSQL.AppendLine("WHERE CSOBREPESO='N'")
                vcSQL.AppendLine("AND EE.CFOLIO_REMIS = '" & prmFolioRemis & "'")
                vcSQL.AppendLine("AND EE.CCVE_TEMPORADA = '" & fgGetParametro("TEMPORADA") & "'")
                vcSQL.AppendLine("AND EE.CMERCADO = 'E'")
                vcSQL.AppendLine("GROUP BY EDE.CCVE_TEMPORADA,EDE.CCVE_EMPAQUE,EDE.CFOLIO_MANIF,EDE.CMERCADO,C.CNOMBRE,CT.CNOMBRE,CE.CNOMBRE,ETI.CNOMBRE ,U.CDESCRIP, U.CABREVIAT,CC.CSUPERMERCADO--,CT.CNOMBRE,CT.caarcnac,CT.caarcexp ")

                If Not fgCreaVista(vcSQL.ToString, "VWDETFACTNAC") Then
                    fgCreaVistasGuia = "Ocurrio un error"
                    Exit Function
                End If

            Catch ex As Exception
                fgCreaVistasGuia = ex.Message
            End Try

        End Function


        Public Shared Function fgNumLetraNormal(ByVal acant) As String
            Dim RNE(21), R100(9), R10(9), R1(9)
            Dim rletra As String
            Dim s As Long
            Dim A100 As Long
            ...
            Do While True
                ...
                End If
            Loop
            fgNumLetraNormal = rletra + " PESOS " + Format(dec, "00") + "/100" + " M.N."
        End Function


        Public Shared Function fgNumLetraUS(ByVal acant) As String
            Dim RNE(21), R100(9), R10(9), R1(9)
            Dim rletra As String
            Dim s As Long
            Dim A100 As Long
            ...
            Do While True
                ...
                End If
            Loop
            fgNumLetraUS = rletra + " US. " + Format(dec, "00") + "/100" + ""
        End Function


        Public Shared Function fgYaSeEnvioMailSistema(ByRef prmSistema As String, ByVal prmFecha As Date) As Boolean

            Dim DT As New DataTable
            Dim vcSQL As String

            fgYaSeEnvioMailSistema = False

            vcSQL = "SELECT * FROM ENVIAINFO(NOLOCK)"
            vcSQL = vcSQL & vbCrLf & "WHERE CSISTEMA = '" & prmSistema & "' AND DFECHA = '" & Format(prmFecha, "yyyyMMdd") & "'"
        ...
        Public Shared Function fgGeneraArchivoElectronicoHM(ByVal prmRuta As String, ByVal prmFolioManif As String) As String


            Dim vcSQL As New System.Text.StringBuilder()
            Dim VLencabezado As String
            Dim VLLINEA As String
            Dim vlmanifiesto As String
            Dim DT As New DataTable

            fgGeneraArchivoElectronicoHM = ""

            Try
                vcSQL.AppendLine("SELECT 'HD' AS E1,'001' AS E2,RIGHT(EE.CCVE_CLIENTE,3) AS E3,LTRIM(RTRIM(EE.NCAJA)) AS E3_1,DBO.FGPONCEROS(MONTH(EE.DFECHA_TRABAJO),2)+'/'+DBO.FGPONCEROS(DAY(EE.DFECHA_TRABAJO),2)+'/'+RIGHT(YEAR(EE.DFECHA_TRABAJO),2) AS E4,")
                vcSQL.AppendLine(" (CONVERT(CHAR(5),EE.DFECHA_TRABAJO,114)) AS E5,'45'AS E6,'C'AS E7,LTRIM(RTRIM(RIGHT(EE.CFOLIO_REMIS,6))) AS E8,")
                vcSQL.AppendLine(" EE.CPLACASCAJA AS E9,'001' AS E10,'PACIFIC BROKERAGE' AS E11,'002' AS E12,")
                vcSQL.AppendLine(" 'MAYER MARTINEZ' AS E13,LTRIM(RTRIM(RIGHT(CFOLIO_FAC,6))) AS E14,'003' AS E15,'113' AS E16,RTRIM(C_SCAC) AS E17,RTRIM(EE.CPLACASCAMION) AS E18,RTRIM(CHO.CLICENCIA) AS E19,")
                vcSQL.AppendLine(" '01' AS V1,RIGHT(ED.CFOLIO_PALET,6) AS V2,ISNULL(CC.CCVE_DISTRIBUIDOR,'')+ISNULL(RIGHT(ED.CCVE_ENVASE,2),'')+ISNULL(RIGHT(ED.CCVE_TAMAÑO,2),'') AS V3,")
                vcSQL.AppendLine(" CC.CNOMBRE+' '+CE.CNOMBRE+' '+CASE WHEN CT.CNOMBRE = 'XXL' THEN 'JUMBO' ELSE CT.CNOMBRE END AS V4,RIGHT(CASE WHEN CLO.CCVE_LOTE IN ('76','77') THEN CASE WHEN ED.CCVE_ETIQUETA = '001' THEN '010' WHEN ED.CCVE_ETIQUETA = '003' THEN '015' END ELSE ED.CCVE_ETIQUETA END,2) AS V5,CASE WHEN CLO.CCVE_LOTE IN ('76','77') THEN ETI.CNOMBRE+'-L1' ELSE CASE WHEN E.CPRODUCE = 'S' THEN 'PE' ELSE ETI.CNOMBRE END END AS V6,")
                vcSQL.AppendLine(" ED.NBULTOS AS V6_1,DBO.FGPONCEROS(ED.NBULTOS*NLIBRAS,5) AS V7,'L' AS V8,'00'+RTRIM(CAST(ED.NPRECIO AS CHAR(10))) AS V9,CASE WHEN ED.CSOBREPESO='S' THEN 'S' ELSE '' END AS V10,")
                vcSQL.AppendLine(" 'F' AS V11,CASE WHEN E.CCHEP='S' THEN 1 ELSE 0 END AS V12,LEFT(LTRIM(RTRIM(CLO.CDESCRIP)),25) AS V13,E.DFECHA AS V14")
                vcSQL.AppendLine(" FROM EYE_ENCEMBARQUES EE(NOLOCK)")
                vcSQL.AppendLine(" LEFT JOIN EYE_DETEMBARQUES ED(NOLOCK)")
                vcSQL.AppendLine(" ON  EE.CCVE_TEMPORADA=ED.CCVE_TEMPORADA")
                vcSQL.AppendLine("     AND EE.CCVE_EMPAQUE=ED.CCVE_EMPAQUE")
                vcSQL.AppendLine("     AND EE.CFOLIO_MANIF=ED.CFOLIO_MANIF AND EE.CMERCADO=ED.CMERCADO")
                vcSQL.AppendLine("LEFT JOIN EYE_EMPAQUE E(NOLOCK) ON E.CCVE_TEMPORADA=ED.CCVE_TEMPORADA AND E.CCVE_EMPAQUE=ED.CCVE_EMPAQUE AND E.CFOLIO_PALET=ED.CFOLIO_PALET")
                vcSQL.AppendLine(" LEFT JOIN CTL_TAMAÑOS   CT(NOLOCK)  ON ED.CCVE_CULTIVO  = CT.CCVE_CULTIVO AND ED.CCVE_TAMAÑO=CT.CCVE_TAMAÑO")
                vcSQL.AppendLine(" LEFT JOIN CTL_ENVASES   CE(NOLOCK)  ON ED.CCVE_ENVASE   = CE.CCVE_ENVASE")
                vcSQL.AppendLine(" LEFT JOIN CTL_ENVASE_BULTOS CEB(NOLOCK) ON ED.CCVE_ENVASE   = CEB.CCVE_ENVASE AND ED.CCVE_CULTIVO=CEB.CCVE_CULTIVO")
                vcSQL.AppendLine(" LEFT JOIN CTL_ETIQUETAS ETI(NOLOCK) ON ED.CCVE_ETIQUETA = ETI.CCVE_ETIQUETA")
                vcSQL.AppendLine(" LEFT JOIN CTL_CULTIVOS  CC(NOLOCK)  ON ED.CCVE_CULTIVO  = CC.CCVE_CULTIVO")
                vcSQL.AppendLine(" LEFT JOIN CTL_LOTES CLO(NOLOCK) ON ED.CCVE_LOTE=CLO.CCVE_LOTE AND ED.CCVE_TEMPORADA=CLO.CCVE_TEMPORADA")
                vcSQL.AppendLine(" LEFT JOIN CTL_LINEASDETRANSPORTE CL(NOLOCK) ON EE.CCVE_LINEA=CL.CCVE_LINEA")
                vcSQL.AppendLine(" LEFT JOIN CTL_CHOFERES CHO(NOLOCK) ON EE.CCVE_CHOFER=CHO.CCVE_CHOFER")
                vcSQL.AppendLine(" WHERE CCVE_CLIENTE=1 AND EE.CCVE_EMPAQUE='" & fgGetParametro("EMPAQUE") & "' AND EE.CCVE_TEMPORADA='" & fgGetParametro("TEMPORADA") & "'")
                vcSQL.AppendLine(" AND EE.CFOLIO_MANIF IN (" & prmFolioManif & ")")
                vcSQL.AppendLine(" ORDER BY ee.cfolio_manif")

                DAO.RegresaConsultaSQL(vcSQL.ToString, DT)
                ...
                If Not DT Is Nothing AndAlso DT.Rows.Count > 0 Then
                    ...
                    Dim oSW As New StreamWriter(prmRuta, False)
                    ...
                    For Each vRow As DataRow In DT.Rows
                        ...
                        If VLencabezado <> vRow("e1") + "|" + vRow("e2") + "|" + vRow("e3") + "|" + vRow("E3_1") + "|" + vRow("e4") + "|" + vRow("e5") + "|" + vRow("e6") + "|" + vRow("e7") + "|" + vRow("e8") + "|" + vRow("e9") + "|" + vRow("e10") + "|" + vRow("e11") + "|" + vRow("e12") + "|" + vRow("e13") + "|" + vRow("e14") + "|" + vRow("e15") + "|" + vRow("e16") + "|" + vRow("e17") + "|" + vRow("e18") + "|" + vRow("e19") Then
                            VLencabezado = vRow("e1") + "|" + vRow("e2") + "|" + vRow("e3") + "|" + vRow("E3_1") + "|" + vRow("e4") + "|" + vRow("e5") + "|" + vRow("e6") + "|" + vRow("e7") + "|" + vRow("e8") + "|" + vRow("e9") + "|" + vRow("e10") + "|" + vRow("e11") + "|" + vRow("e12") + "|" + vRow("e13") + "|" + vRow("e14") + "|" + vRow("e15") + "|" + vRow("e16") + "|" + vRow("e17") + "|" + vRow("e18") + "|" + vRow("e19")
                            oSW.WriteLine(VLencabezado)
                            oSW.Flush()

                        End If
                        VLLINEA = vRow("v1") + "|" + vRow("v2") + "|" + ("" & vRow("v3")) + "|" + vRow("v4") + "|" + vRow("v5") + "|" + vRow("v6") + "|" & vRow("v6_1") & "|" & Int(vRow("v7")) & "|" + vRow("v8") + "|1|" + vRow("v9") + "|" + vRow("v10") + "|" + vRow("v11") + "|" + Trim(Str(vRow("v12"))) + "|" + Trim(vRow("v13")) + "|" + Trim(Format(vRow("v14"), "MM/dd/yy"))
                        oSW.WriteLine(VLLINEA)
                        oSW.Flush()
                    Next
            ...
            Catch ex As Exception
                fgGeneraArchivoElectronicoHM = ex.Message
            End Try

        End Function


        Public Shared Function fgCreaManifiestoMarengo(ByVal prmAgricultor As String, ByVal prmFechaInicio As Date, ByVal prmFechaFin As Date, ByVal prmRutaArchivo As String, ByVal prmManifMarengo As String, ByVal prmCodigoRancho As String, ByVal prmCodigoTemporada As String) As String
            ...
            Dim vlTotalR As Integer
            Dim vlTotalP As Integer
            Dim vcRemisIni As String
            Dim vcRemisFin As String

            Dim vlPARTNERID As Long
            Dim vlOPTTYPE As Long
            Dim vlTAGUID As String
            Dim vlTAGNUM As String
            Dim vlGROWERCODE As String
            Dim vlCONTRACT As Long
            Dim vlRANCHFIELDID As String
            Dim vlClaveMarengo As String


            Try

                vcSQL = "SELECT CGROWERIDMARENGO,CPARTNERIDMARENGO "
                vcSQL = vcSQL & vbCrLf & "FROM CTL_AGRICULTORES(NOLOCK)"
                vcSQL = vcSQL & vbCrLf & "WHERE CCVE_AGRICULTOR = '" & prmAgricultor & "'"

                DAO.RegresaConsultaSQL(vcSQL, DTTrabajo)

                If Not DTTrabajo Is Nothing AndAlso DTTrabajo.Rows.Count > 0 Then
                    vlGROWERCODE = DTTrabajo.Rows(0)("CGROWERIDMARENGO")
                    vlPARTNERID = DTTrabajo.Rows(0)("CPARTNERIDMARENGO")
                    vlTAGUID = DTTrabajo.Rows(0)("CGROWERIDMARENGO")
                    vlTAGNUM = DTTrabajo.Rows(0)("CGROWERIDMARENGO")
                End If
                DTTrabajo = Nothing
                ...
                vcSQL = ""
                vcSQL = vcSQL & vbCrLf & "SELECT COALESCE(count(de.cfolio_manif),0) as TotalRegistros, sum(de.nbultos) as TotalPaquetes,"
                vcSQL = vcSQL & vbCrLf & "min(enc.cfolio_remis) as minimo,max(enc.cfolio_remis) as maximo"
                vcSQL = vcSQL & vbCrLf & "FROM eye_detembarques de(nolock) "
                vcSQL = vcSQL & vbCrLf & "JOIN eye_encembarques enc(nolock) on enc.ccve_temporada = de.ccve_temporada"
                vcSQL = vcSQL & vbCrLf & "AND de.cfolio_manif = enc.cfolio_manif AND de.ccve_empaque = enc.ccve_empaque"
                vcSQL = vcSQL & vbCrLf & "JOIN ctl_clientes cte(NOLOCK) ON enc.ccve_cliente = cte.ccve_cliente"
                vcSQL = vcSQL & vbCrLf & "WHERE enc.ccve_empaque = '" & fgGetParametro("EMPAQUE") & "'"
                vcSQL = vcSQL & vbCrLf & "AND enc.ccve_temporada = '" & fgGetParametro("TEMPORADA") & "'"
                vcSQL = vcSQL & vbCrLf & "AND de.ccve_agricultor = '" & prmAgricultor & "'"
                vcSQL = vcSQL & vbCrLf & "AND cte.cclientemarengo = 'S' "
                vcSQL = vcSQL & vbCrLf & "AND Convert(VARCHAR(10),enc.dfecha_trabajo,112) BETWEEN '" & Format(prmFechaInicio, "yyyyMMdd") & "' AND '" & Format(prmFechaFin, "yyyyMMdd") & "' "

                vcSQL = vcSQL & vbCrLf & "AND ENC.CFOLIO_MANIF IN (" & prmManifMarengo & ")"
                ...
                If Not DTTrabajo Is Nothing AndAlso DTTrabajo.Rows.Count > 0 Then
                    If DTTrabajo.Rows(0)("TOTALREGISTROS") = 0 Then
                        fgCreaManifiestoMarengo = "No Existen Datos para Generar este Archivo, Favor de Verificar"
                        DTTrabajo = Nothing
                        Exit Function
                    End If

                    vlTotalR = DTTrabajo.Rows(0)("TOTALREGISTROS")
                    vlTotalP = DTTrabajo.Rows(0)("TotalPaquetes")
                    vcRemisIni = Val(DTTrabajo.Rows(0)("minimo"))
                    vcRemisFin = Val(DTTrabajo.Rows(0)("MAXIMO"))
                End If

                DTTrabajo = Nothing

                vcSQL = ""
                vcSQL = vcSQL & vbCrLf & "SELECT Right(ee.cfolio_remis,4) AS manifiesto,ee.cfolio_manif as folio, "
                vcSQL = vcSQL & vbCrLf & "ee.dfecha_trabajo as fechaembarque,ee.ncaja as truckid,ee.cplacascamion as placas,"
                vcSQL = vcSQL & vbCrLf & "left(cnom_chofer,25) as conductor, "
                vcSQL = vcSQL & vbCrLf & "(select left(CDESCEMBARQUE,30) from ctl_cultivos where ccve_cultivo = de.ccve_cultivo) as producto, "
                vcSQL = vcSQL & vbCrLf & "(select left(cnombre,15) FROM CTL_TAMAÑOS (nolock) where ccve_cultivo = de.ccve_cultivo and ccve_tamaño = de.ccve_tamaño) as tamano, "
                vcSQL = vcSQL & vbCrLf & "(select bultosxpallet from ctl_envases ce(nolock) right join CTL_ENVASE_BULTOS CEB(NOLOCK) ON CEB.CCVE_ENVASE=CE.CCVE_ENVASE where ceb.ccve_cultivo = de.ccve_cultivo and ceb.ccve_envase = de.ccve_envase) as bultosxpallet, "
                vcSQL = vcSQL & vbCrLf & "(select left(cnombre,15) FROM CTL_ENVASES (nolock) where ccve_envase = de.ccve_envase) as envase, "
                vcSQL = vcSQL & vbCrLf & "(select left(cnombre,20) FROM CTL_ETIQUETAS (nolock) where ccve_etiqueta = de.ccve_etiqueta) as etiqueta, "
                vcSQL = vcSQL & vbCrLf & "de.cfolio_palet,de.ccve_cultivo,de.ccve_tamaño,de.ccve_envase,de.ccve_etiqueta,de.nbultos,COALESCE(MAR.CCVE_MARENGO,'') AS CCVE_MARENGO,"
                vcSQL = vcSQL & vbCrLf & "emp.dfecha as DFECHAEMPAQUE"
                vcSQL = vcSQL & vbCrLf & "FROM   eye_encembarques ee(nolock) "
                vcSQL = vcSQL & vbCrLf & "JOIN eye_detembarques de(nolock) on de.ccve_temporada = ee.ccve_temporada"
                vcSQL = vcSQL & vbCrLf & "AND de.ccve_empaque = ee.ccve_empaque AND de.cfolio_manif = ee.cfolio_manif"

                vcSQL = vcSQL & vbCrLf & "JOIN eye_empaque emp(nolock) ON emp.ccve_temporada = de.ccve_temporada AND emp.ccve_empaque = de.ccve_empaque"
                vcSQL = vcSQL & vbCrLf & "AND emp.cfolio_palet = de.cfolio_palet"

                vcSQL = vcSQL & vbCrLf & "JOIN CTL_CLIENTES CTE(NOLOCK) ON EE.CCVE_CLIENTE = CTE.CCVE_CLIENTE"

                vcSQL = vcSQL & vbCrLf & "JOIN EYE_CONFLOTESMARENGO MAR(NOLOCK) ON MAR.CCVE_TEMPORADA = DE.CCVE_TEMPORADA AND MAR.CCVE_LOTE = DE.CCVE_LOTE"
                vcSQL = vcSQL & vbCrLf & "AND MAR.CCVE_CULTIVO = DE.CCVE_CULTIVO"



                vcSQL = vcSQL & vbCrLf & "WHERE  ee.ccve_empaque = '" & fgGetParametro("EMPAQUE") & "' "
                vcSQL = vcSQL & vbCrLf & "AND ee.ccve_temporada = '" & fgGetParametro("TEMPORADA") & "' "
                vcSQL = vcSQL & vbCrLf & "AND de.ccve_agricultor = '" & prmAgricultor & "'"
                vcSQL = vcSQL & vbCrLf & "AND CTE.CCLIENTEMARENGO = 'S' "

                vcSQL = vcSQL & vbCrLf & "AND Convert(VARCHAR(10),ee.dfecha_trabajo,112) BETWEEN '" & Format(prmFechaInicio, "yyyyMMdd") & "' AND '" & Format(prmFechaFin, "yyyyMMdd") & "' "

                vcSQL = vcSQL & vbCrLf & "AND EE.CFOLIO_MANIF IN (" & prmManifMarengo & ")"

                vcSQL = vcSQL & vbCrLf & "ORDER BY manifiesto,PRODUCTO,DE.NPOSICION"
                ...
                Dim oSW As New StreamWriter(prmRutaArchivo, False)

                'Start File

                'The File Record
                vlCadena = "///+"    'Begin of line 1 of 4
                vlCadena = vlCadena & "CONTENT+LIT+"    'Content 2 of 12
                vlCadena = vlCadena & "VERSION+3.00+"    'File Version 3 of 13
                vlCadena = vlCadena & "PARTNERID+"    'PARTNERID 4 of 10
                vlCadena = vlCadena & vlPARTNERID    '** Partner id 5 of (Must be provided by the company)
                vlCadena = vlCadena & "+///"    'End of line 6 of 4

                oSW.WriteLine(vlCadena)
                oSW.Flush()

                'The Header Record
                vlCadena = "///+"    'Begin of line 1 of 4
                vlCadena = vlCadena & "TOTREC+"    'TOTREC 2 of 7
                vlCadena = vlCadena & vlTotalR    'Total of records 3 (Total number of records, without file, header and trailer records, included in the file)
                vlCadena = vlCadena & "+"    'Separator 4 of 1
                vlCadena = vlCadena & "TOTQTY+"    'TOTQTY 5 of 7
                vlCadena = vlCadena & vlTotalP    'Total quantity 6 of (Total quantity of packages included in the file - Optional. Total packages to import)
                vlCadena = vlCadena & "+OPTYPE+"    'OPTTYPE 7 of 7
                vlCadena = vlCadena & vlOPTTYPE    '* Operation Type 8 of (Operation type number described below)
                vlCadena = vlCadena & "+"    'Separator 9 of 1
                vlCadena = vlCadena & "TAGUID+"    'TAGUID 10 of 7
                vlCadena = vlCadena & vlTAGUID    '** Tag UID 11 of 3 digits (Tag UID. Necessary when the tags are generated from the partner and will continue in use after receiving "Must be provided by the company"
                vlCadena = vlCadena & "+"    'Separator 12 of 1
                vlCadena = vlCadena & "FLDDEL+"    'FLDDEL 13 of 7
                vlCadena = vlCadena & "*"    'Used separator sign 14 of 1 (A single character used as "field delimiter" for the "detail records". E.g. asterisk, tilde, etc)
                vlCadena = vlCadena & "+///"    'End of line 15 of 4
                ...
                For Each vRow As DataRow In DTTrabajo.Rows
                    ...
                    'The Detail Records
                    vlCadena = "REC="
                    vlCadena = vlCadena & fgPonceros(Trim(vRow("manifiesto")), 6)    'Manifiest 1 Char max 6
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & vlGROWERCODE    '** Grower Code 2 of Char 3 (Must be provided by the company)
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & fgPonceros(Trim(vRow("manifiesto")), 6)    'Grower Manifiest 3 of Max 6
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & vlCONTRACT    '** Contract 4
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & Format(vRow("fechaembarque"), "yyyyMMdd")    'Ship date 5 Format YYYYMMDD (Date when the product left its warehouse in origin with destination to the company warehouse)
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & Format(vRow("fechaembarque"), "HH:mm")    'Ship time 6 24hrs Format HH:mm (Time when the product left its warehouse in origin with destination to the company warehouse)
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & Trim(vRow("truckid"))    'Truck ID 7 of Char 25 Max (Truck identification)
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & Trim(vRow("conductor"))    'Driver 8 of Char 25 Max (Driver first and last name)
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & Trim(vRow("placas"))    'Trailer Lic 9 of Char 12 Max
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & ""    'For future use 10

                    If vlOPTTYPE = 3 Then
                        vlCadena = vlCadena & "*"
                        vlCadena = vlCadena & prmCodigoTemporada & vlTAGNUM + Trim(vRow("CFOLIO_PALET"))    '*** Tag No 11 of Numeric 18 digits (Information will be used only if the "operation type" specified in the "header record" is set to 3)
                    End If

                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & Trim(vRow("CCVE_CULTIVO")) + "-" + Trim(vRow("CCVE_TAMAÑO")) + "-" + Trim(vRow("ccve_envase")) + "-" + Trim(vRow("CCVE_ETIQUETA"))    '* Product ID 12 of Char 25 Max (Product Identification. Read more below1)
                    vlCadena = vlCadena & "*"

                    'vlCadena = vlCadena & vlRANCHFIELDID    '** Ranch-Field ID 13 of Char 6 (Composed as follows, First 3 = Ranch Code. Left spaces will be trim, Last 3 = Field Code, Left spaces will be trim)
                    vlCadena = vlCadena & vlRANCHFIELDID & prmCodigoRancho & vRow("CCVE_MARENGO")   '** Ranch-Field ID 13 of Char 6 (Composed as follows, First 3 = Ranch Code. Left spaces will be trim, Last 3 = Field Code, Left spaces will be trim)

                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & Trim(vRow("NBULTOS"))    'Quantity 14 of Numeric (Total quantity (of packages) for the corresponding "Product ID")
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & "" & Format(vRow("DFECHAEMPAQUE"), "yyyyMMdd")    'Pack Date Date when the product was packed by the grower. Only processed when operation type is 2 or 3
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & Trim(vRow("BULTOSXPALLET"))    'Units per pallet 16 of Numeric (Indicates how many packages per pallet are packed for the specific product) Read more below2
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & ""    'Reference 1 17 of Char 10 Max
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & ""    'Reference 2 18 of Char 4 Max
                    vlCadena = vlCadena & "*"
                    vlCadena = vlCadena & Trim(vRow("PRODUCTO")) + " " + Trim(vRow("tamano")) + " " + Trim(vRow("envase")) + " " + Trim(vRow("Etiqueta")) + " " + Trim(vRow("BULTOSXPALLET")) + " Cajas"    'Product Description 19 of Char 80 Max
                    vlCadena = vlCadena & "<<<"
                ...
                fgCreaManifiestoMarengo = ""

            Catch ex As Exception
                fgCreaManifiestoMarengo = ex.Message
            End Try
        ...
        Public Shared Function fgHoyTime() As String

            Dim DT As New DataTable
            Dim vcSQL As String

            vcSQL = "select 'cHora'=right('00'+cast(datepart(hh, getdate()) as varchar(02)),2)+':'+right('00'+cast(datepart(mi, getdate()) as varchar(02)),2)"

            DAO.RegresaConsultaSQL(vcSQL, DT)

            If Not DT Is Nothing Then
                If DT.Rows.Count > 0 Then
                    fgHoyTime = DT.Rows(0)("cHora")
                End If
            End If
        ...
        Public Shared Function ProveedoresAgroquimicos(ByVal prmRFC As String) As Boolean
            ...
            Try
                vcSQL = "SELECT * FROM PROVAGROQUIMICOS WHERE CRFC = '" & prmRFC & "'"
        ...
        Public Shared Function fgTraeEnfriados(ByVal prmTemporada As String, ByVal prmFecha As DateTime) As DataSet
            ...
            vcSQL = "SPPALLETSPREENFRIADO"

            Try
                DAO.RegresaConsultaSQL(vcSQL, DS, vParam)
                If Not DS Is Nothing Then
                    Return DS
                End If
            Catch ex As Exception
        ...
        Public Shared Function fgTraeVanceLabores(ByVal prmTemporada As String, ByVal prmNomina As String, ByVal prmFecha As DateTime) As DataSet
        ...
        Public Shared Function fgTraeInfoTractoresRenta(ByVal prmTemporada As String, ByVal prmNomina As String, ByVal prmFecha As DateTime) As DataSet
            Dim vParam(2) As Object
            Dim DS As New DataSet
        ...
        Public Shared Function fgTraeVentasDiarias(ByVal prmTemporada As String, ByVal prmFecha As DateTime) As DataSet
            Dim vParam(1) As Object
            Dim DS As New DataSet
        ...
        Public Shared Function fgTraeVentasDiariasMarengo(ByVal prmTemporada As String, ByVal prmFecha As DateTime) As DataSet
            Console.WriteLine("### Continua Proceso: fgTraeVentasDiariasMarengo ### ")
            Dim vParam(1) As Object
            Dim DS As New DataSet
        ...
        Public Shared Function fgTraeEmpleadosMayoresDeEdad(ByVal prmTemporada As String) As DataSet
            Dim vParam(1) As Object
            Dim DS As New DataSet
        ...
        Public Shared Function fgTraeExistenciasDistribuidoras(ByVal prmTemporada As String, ByVal prmFecha As DateTime) As DataSet
            Dim vParam(1) As Object
            Dim DS As New DataSet
            ...
            vcSQL = "SPINVENTARIOSDISTRIBUIDORAS"
        ...
        Public Shared Function fgTraeRotacionDistribuidoras(ByVal prmTemporada As String, ByVal prmCultivo As String, ByVal prmFechaIni As DateTime, ByVal prmFechaFin As DateTime) As DataTable
            Dim vParam(3) As Object
            Dim DS As New DataSet

            vParam(0) = prmTemporada
            vParam(1) = prmCultivo
            vParam(2) = prmFechaIni
            vParam(3) = prmFechaFin

            Dim vcSQL As String = ""

            vcSQL = "SPOBTENROTACIONPRODUCTOSDISTRIBUIDORAS"
